...
resource "google_sql_database" "database" {
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "(sudo docker run  --rm --interactive --mount=type=bind,source=${data.vault_generic_secret.secret_manager_key.data["value"]},destination=/tmp/secret_manager_key.json --mount=type=bind,source=$PWD/store_database_credentials.sh,destination=/tmp/store_database_credentials.sh google/cloud-sdk /bin/bash /tmp/store_database_credentials.sh  --env=migrator='${google_sql_database.database.name}_migrator' --env=migrator_pass=${random_password.migrations_user_pass.result} --env=app_user='${google_sql_database.database.name}_app' --env=app_user_pass=${random_password.apps_user_pass.result} --variable=analytics_user='${google_sql_database.database.name}_analytics' --variable=analytics_user_pass=${random_password.analytics_user_pass.result})"
    ]
  }
}
