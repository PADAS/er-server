...
When the reference is resolved (for example in a pre-rendered event type schema), dynamic schema endpoints expand to an **`enum`** list of allowed values plus **`x-enumExtra`**: an object keyed by each enum value whose values hold `title`, optional `description`, and any extra keys requested via the `enum_extra` query parameter. This keeps validators from compiling thousands of `oneOf`/`anyOf` branches while preserving display metadata.
