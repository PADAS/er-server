...
resource "template_dir" "deployments" {
  ...
  vars = {
    ...
    UBI_API_USERNAME                = data.vault_generic_secret.ubi_api_credentials.data.username
    UBI_API_PASSWORD                = data.vault_generic_secret.ubi_api_credentials.data.password
  }
}
