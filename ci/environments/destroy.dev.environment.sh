...
if [ $# -eq 0 ]; then
    ...
    echo "USAGE:"
    echo "  1: ENVIONMENT_NAME=REQUIRED What is the name of the envionment to destroy: This will be branch name/cluster name/pipeline name"
...
# Delete CI Pipeline
##################################################################################
fly -t $PROJECT dp -p $ENVIONMENT_NAME

##################################################################################
# Delete Cluster
##################################################################################
INFRA_DIR_X="$DEV_CREATE_DIR/../../../infrastructure"
$INFRA_DIR_X/resources/k8s/delete.gcp.cluster.sh $PROJECT $ENVIONMENT_NAME

##################################################################################
# Delete Disks
##################################################################################
DEFAULT_DISKS_LOCATION="$DEV_CREATE_DIR/../../deployment/storage/default.disks.csv"
$INFRA_DIR_X/resources/cloud/gcp/disks/delete.from.csv.sh $PROJECT $ENVIONMENT_NAME $DEFAULT_DISKS_LOCATION
