...
CI_INCLUDE_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"

if [ $# -eq 0 ]; then
    echo "This script is intended to set a deploy only pipeline for the given branch/cluster/pipeline name"
...
source $CI_INCLUDE_DIR/pipeline.base.sh

set-pipeline $PIPELINE $PIPELINE_TYPE "deploy-to-" $OVERRIDES
