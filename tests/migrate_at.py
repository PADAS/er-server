...
# Can't import models until django.setup() has been called
...
def import_trackinguser(userid):
    ...
    # Visible observation range
    range_begin = trackinguser.get('fulldataaccess', -999)
    range_end = trackinguser.get('delay', 0)

    if range_begin >= 0:
        begin_pset = None
        begin_pset_name = "access_begins_" + str(range_begin)
        q_begin_pset = accounts.models.PermissionSet.objects.get_or_create(name=begin_pset_name)
        if q_begin_pset is not None and len(q_begin_pset) > 0 and q_begin_pset[0] is not None:
            begin_pset = q_begin_pset[0]

        if begin_pset is not None:
            das_user.permission_sets.add(begin_pset)

    if range_end >= 0:
        end_pset = None
        end_pset_name = "access_ends_" + str(range_end)
        q_end_pset = accounts.models.PermissionSet.objects.get_or_create(name=end_pset_name)
        if q_end_pset is not None and len(q_end_pset) > 0 and q_end_pset[0] is not None:
            end_pset = q_end_pset[0]

        if end_pset is not None:
            das_user.permission_sets.add(end_pset)
...
def import_all_chronofiles():
    ...
    for animal in rows:
        try:
            ...
            chronofile_error_list.append({'User {0} import error - {1}'.format(animal.chronofile, ex.message)})
...
def import_all_users():
    ...
    for user in rows:
        try:
            ...
            user_error_list.append({'User {0} import error - {1}'.format(user.userid, ex.message)})
