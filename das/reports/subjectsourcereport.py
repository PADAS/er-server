    ...
    for ss in SubjectSource.objects.filter(
        ...
        if latest_observation:
            ...
            latest_observations = Observation.objects.filter(
                source=ss.source, recorded_at__gt=(latest_observation.recorded_at - timedelta(hours=report_hours))
            )

            alert_accumulator = {}
            for ar in SubjectAnalyzerResult.objects.filter(
                subject=ss.subject,
                estimated_time__range=(
                    latest_observation.recorded_at - timedelta(hours=report_hours),
                    latest_observation.recorded_at,
                ),
                level__gt=SubjectAnalyzerResult.LEVEL_OK,
            ):
                if ar.subject_analyzer:
                    k = ar.subject_analyzer.analyzer_category
                    alert_accumulator.setdefault(k, 0)
                    alert_accumulator[k] += 1

            try:
                trajectory = ss.subject.create_trajectory(
                    obs=latest_observations, trajectory_filter_params=ss.subject.default_trajectory_filter()
                )
                trajectory_length = trajectory.relocs.fix_count
            except Exception:
                trajectory_length = "n/a"

            result["performance"] = (len(latest_observations), trajectory_length)

            result["voltage"] = (
                latest_observation.additional.get("voltage", "") if latest_observation.additional else ""
            )

            result["analyzers"] = alert_accumulator
            result["analyzers_summary"] = ", ".join("{}({})".format(k, v) for k, v in alert_accumulator.items())

            result["time_since_last"] = calculate_age_description(latest_observation.recorded_at)

            result["styles"] = {}
            los_styles = calculate_styles("latest_observation_at", latest_observation.recorded_at)
...
def filter_by_user(values, user, key="subject_id"):
    ...
    """
    user_subject_ids = [sub.id for sub in Subject.objects.all().by_user_subjects(user)]
...
def groupify_report_data(subject_records):
    ...
    try:
        for record in subject_records:
            ...
            group = groups.setdefault((species, region), {"species": species, "region": region})
            # Used in template.
    ...
    group_list = sorted(groups.values(), key=lambda x: "%s %s" % (species, region), reverse=False)
