...
def get_rhinos():
    rhinos = Subject.objects.filter(
        subject_type=Subject.TYPE_WILDLIFE, subject_subtype=Subject.SUBTYPE_RHINO)
    return rhinos
...
def get_daily_report_data(since, before, **kwargs):
    ...
    context = {
        'report_filename': 'Daily-SitRep-{}.docx'.format(before.astimezone(timezone.get_current_timezone())
                                                         .strftime('%Y-%m-%d')),
        'report_time': before.astimezone(timezone.get_current_timezone()).strftime('%-d %B %Y %Z'),
        'report_daterange_text': 'Including events from: {} to: {}'.format(since_text, before_text),
        'footer_text': 'Report generated by DAS user {username} at {generated_at}'.format(
