...
def get_daily_report_data(since, before, event_categories=None, **kwargs):
    ...
    def fence_breakage(accum, event):
        ...
        rendered_schema = get_rendered_schema_properties(event.event_type)
        etime = event.event_time.astimezone(get_current_timezone())
        b = {
        ...
        def inner_events(accum, event):
            ...
            def build_note(note):
                return {
                    ...
                    "created_at": note.created_at.astimezone(get_current_timezone()).strftime(
            ...
            accum.append(
                {
                    ...
                    "event_time": event.event_time.astimezone(get_current_timezone()).strftime(
                        EVENT_LIST_TIMESTAMP_FORMAT
                    ),
    ...
    def human_wildlife_conflict(accum, event):
        ...
        def build_note(note):
            return {
                "text": html.escape(note.text),
                "username": note.created_by_user.username,
                "created_at": note.created_at.astimezone(get_current_timezone()).strftime(EVENT_LIST_TIMESTAMP_FORMAT),
            }
        ...
        accum.append(
            {
                "title": "{}: {}".format(event.serial_number, escape(event.title)),
                "event_name": "{}: {}".format(event.serial_number, escape(event.title)),
                "event_time": event.event_time.astimezone(get_current_timezone()).strftime(EVENT_LIST_TIMESTAMP_FORMAT),
                "attributes": sorted(event_details, key=lambda x: x["order"]),
                "notes": [build_note(n) for n in en],
            }
        )
    ...
    since_text = since.astimezone(get_current_timezone()).strftime(REPORT_TIMESTAMP_FORMAT)
    before_text = before.astimezone(get_current_timezone()).strftime(REPORT_TIMESTAMP_FORMAT)
    context = {
        "report_filename": "Daily-SitRep-{}.docx".format(
            before.astimezone(get_current_timezone()).strftime("%Y-%m-%d")
        ),
        "report_time": before.astimezone(get_current_timezone()).strftime(REPORT_TIME_FORMAT),
