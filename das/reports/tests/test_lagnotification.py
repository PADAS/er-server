...
def generate_random_positions(
    source, min_lag_mins=10, max_lag_mins=20, x=37.5, y=0.56, time_length=timedelta(minutes=30), intervals=10
):
    -time_length
...
class TestSubjectSourceReport(TestCase):
    def setUp(self):
        ...
        pset = PermissionSet.objects.get(permissions__codename=OBSERVATION_LAG_NOTIFY_PERMISSION_CODENAME)
        for u in (self.u1, self.u2):
            u.permission_sets.add(pset)

        provider1 = SourceProvider.objects.create(
            provider_key="dummy1",
            display_name="Dummy provider1",
            additional=dict(lag_notification_threshold="00:30:00"),
        )

        provider2 = SourceProvider.objects.create(
            provider_key="dummy2", display_name="Dummy provider2"
        )  # using default lag threshold

        provider3 = SourceProvider.objects.create(
            provider_key="dummy3",
            display_name="Dummy provider3",
            additional=dict(lag_notification_threshold="00:10:00"),
        )
        # Add Source
        source1 = Source.objects.create(manufacturer_id="source1", provider=provider1)
        source2 = Source.objects.create(manufacturer_id="source2", provider=provider2)
        source3 = Source.objects.create(manufacturer_id="source3", provider=provider3)
    ...
    def test_lag_notify_permission(self):
        ps = Permission.objects.filter(codename=OBSERVATION_LAG_NOTIFY_PERMISSION_CODENAME)
        self.assertIsNotNone(ps)

        recipients = get_users_for_permission(OBSERVATION_LAG_NOTIFY_PERMISSION_CODENAME)
    ...
    def test_check_source_provider_lag_exceeded_is_empty(self):
        provider_lag_check_data = {"avg_lag": timedelta(hours=5)}

        provider_lag_config = {"lag_notification_threshold": None}
        assert not check_source_provider_lag_exceeded(provider_lag_check_data, provider_lag_config)

        provider_lag_config = {"lag_notification_threshold": ""}
        assert not check_source_provider_lag_exceeded(provider_lag_check_data, provider_lag_config)

        provider_lag_config = {"lag_notification_threshold": " "}
        assert not check_source_provider_lag_exceeded(provider_lag_check_data, provider_lag_config)

        provider_lag_config = {"lag_notification_threshold": " invalid "}
        assert not check_source_provider_lag_exceeded(provider_lag_check_data, provider_lag_config)
...
class TestReportByTask:
    ...
    def test_neither_sources_with_same_provider_reach_the_provider_threshold(self, five_subject_sources):
        provider = five_subject_sources[0].source.provider
        provider.additional = {"silence_notification_threshold": "00:30:00"}
        provider.save()
        source_a = five_subject_sources[0].source
    ...
    def test_two_source_with_same_provider_reach_the_provider_default_threshold(self, five_subject_sources):
        provider = five_subject_sources[0].source.provider
    ...
    def test_two_source_with_different_providers_reach_the_provider_default_threshold(self, five_subject_sources):
        ...
        provider_a.additional = {"default_silent_notification_threshold": "00:30"}
        provider_a.save()
        provider_b = five_subject_sources[1].source.provider
        provider_b.additional = {"default_silent_notification_threshold": "00:30"}
        provider_b.save()
