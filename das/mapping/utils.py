...
TYPE_PROVENANCE_FIELDS = ("last_edited_user", "last_edited_date", "other_id")
...
ATTRIBUTES_TO_SPATIAL_MAPPING = {
    "short_name": {"field": "short_name", "validator": shortname_validator},
...
def mappingv2_save_spatial_data(feature, external_id, spatialfile, counter=0):
    ...
    data = {"external_id": external_id, "feature_geometry": feature_geometry, "feature_type": feature_type}
    ...
    attributes = {
        feature_name: feature[feature_name].value for feature_name in fields if feature_name in attribute_fields
    }
    attributes = reduce_json(attributes)

    provenance = {
        feature_name: feature[feature_name].value for feature_name in fields if feature_name in PROVENANCE_FIELDS
    }
    provenance = reduce_json(provenance)
    source = getattr(spatialfile, "source", DEFAULT_SOURCE_NAME)

    defaults = {"attributes": attributes, "provenance": provenance, "external_source": source}
    for attribute_field, spatial_field in ATTRIBUTES_TO_SPATIAL_MAPPING.items():
        if attribute_field in fields:
            defaults[spatial_field["field"]] = spatial_field["validator"](feature[attribute_field].value)
    ...
    if "tags" in feature.fields:
        feature_record.tags = [value.strip() for value in feature["tags"].value.split(",")]
...
def import_layer(layer, spatialfile):
    logger.info("Importing layer: %s, type: %s, fields: %s", layer.name, layer.geom_type, layer.fields)
...
def get_featuretype_for_feature(feature, default=None):
    ...
    if type_name:
        featuretype = models.FeatureType.objects.get_by_natural_key(type_name)
        return featuretype
    else:
...
def import_feature_types(datasource, source_name=DEFAULT_SOURCE_NAME):
    ...
    for feature in datasource:
        ...
        provenance = {
            feature_name: feature[feature_name].value
            for feature_name in fields
            if feature_name in TYPE_PROVENANCE_FIELDS
        }
        provenance = reduce_json(provenance)
        ...
        if attribute_schema:
            try:
                ...
                logger.warning("FeatureType attribute_schema not JSON for globalid=%s: %s", global_id, ex)
                attribute_schema = {}

        defaults = {"provenance": provenance, "attribute_schema": attribute_schema, "external_source": source_name}

        if "tags" in fields:
            type_record.tags = [value.strip() for value in feature["tags"].value.split(",")]
        for key, value in defaults.items():
            setattr(type_record, key, value)

        type_record.save()
        logger.debug("Import feature_type: %s, created:%s", global_id, created)


def clear_features(obj):
    # Incase of a new spatialfile clear initially created features
    tables = [models.SpatialFeature, models.LineFeature, models.PointFeature, models.PolygonFeature]
    for table in tables:
        try:
