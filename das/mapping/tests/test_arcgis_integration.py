...
from mapping.esri_integration import (
    arcgis_authentication,
    extract_features,
    get_mb_style,
    import_featuretype_presentation,
    search_groups,
)
...
class TestArcGisIntegration(BaseAPITest):
    def setUp(self):
        ...
        self.test_config = ArcgisConfiguration.objects.create(
            config_name="test_config", username="test_username", password="test_pass"
        )
        # Create a GIS account
        self.gis_account = MockGIS(None, username=self.test_config.username, password=self.test_config.password)

        self.create_groups()
        self.gis_group = Group.instances[0]

        self.arcgis_item = ArcgisItem.objects.create(
            id=self.gis_group.id, name=self.gis_group.title, arcgis_config=self.test_config
        )
    ...
    def extract_wfs_features(self, data):
        ...
        SpatialFeature.objects.filter(arcgis_item=self.arcgis_item).exclude(external_id__in=extracted_ids).delete()
