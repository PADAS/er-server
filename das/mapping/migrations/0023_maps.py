...
maps_to_delete = ["5b480df1-dea2-4536-9a3f-03ce5b825abe", "41814d8e-5811-410c-a195-a4ebee9dbf5a"]
...
def create_tile_type_choice(app_registry, db_alias, value, display):
    ...
    choice, created = Choice.objects.using(db_alias).get_or_create(
        model=model, field=field, value=value, defaults=dict(display=display)
    )
...
def update_existing_tilelayer_conf(app_registry, schema_editor):
    ...
    for service in TILE_TYPE_CHOICES:
        create_tile_type_choice(app_registry, db_alias, service["value"], service["display"])
    ...
    google_layer, created = tile_layer.objects.using(db_alias).get_or_create(
        pk=google_sat_pk, defaults=dict(name=Google_satellite_conf["title"], attributes=Google_satellite_conf)
    )
    if not created:
    ...
    mapbox_layer, created = tile_layer.objects.using(db_alias).get_or_create(
        pk=mapbox_sat_pk, defaults=dict(name=Mapbox_satellite_conf["title"], attributes=Mapbox_satellite_conf)
    )
    if not created:
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(update_existing_tilelayer_conf, reverse_code=migrations.RunPython.noop)]
