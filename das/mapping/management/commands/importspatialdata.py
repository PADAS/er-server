...
class Command(TenantCommandMixin, BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for input_filename in self.filename + [
            self.feature_types_file,
        ]:
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "spatialdata_version",
            type=str,
            help="supported versions are {0}, {1}".format(self.SPATIALDATA_VERSIONS[0], self.SPATIALDATA_VERSIONS[1]),
        )
        parser.add_argument("filename", type=str, nargs="*", help="spatial filename")

        parser.add_argument("--featuretype", type=str, help="Feature type")
        parser.add_argument("--featureset", type=str, help="FeatureSet")
        parser.add_argument("--source", type=str, help=f"Source of data, default is {DEFAULT_SOURCE_NAME}")
        parser.add_argument("--layer", type=int, help="Layer to import")
        parser.add_argument("--name-field", type=str, help="Name Field from the attributes table")
        parser.add_argument("--id-field", type=str, help="ID field for the row")
        parser.add_argument("--utm", type=str, help="Change to this utm")
        parser.add_argument("--spatialfile-id", type=str, help="Spatial file ID")
        parser.add_argument("--feature-types", help="spatial feature types file")

    def import_spatial_v1(self):
        if not self.featureset or not self.featuretype:
            logger.info(
                "Ensure both featureset and featuretype are included in command, add flags --featureset and --featuretype"
            )
            return
