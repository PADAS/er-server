...
from mapping.utils import (
    contains_unique_keys_in_layer,
    geometry_mapper,
    get_datasource_and_layer_num,
    get_or_create_feature,
    get_or_create_spatial_feature_type,
    get_spatial_feature_type_name,
    make_external_id,
    set_feature_name,
)
...
DEFAULT_IMAGE = {"image": "/static/ranger_post-black.svg", "width": 20, "height": 20}
...
def arcgis_integration(request, obj):
    ...
    if gis:
        ...
        if "_testconnection" in request.POST:
            message(request, messages.INFO, "Successful Configuration")
...
def update_db_groups(wfs_groups, obj):
    ...
    for group in wfs_groups:
        models.ArcgisGroup.objects.get_or_create(name=group.title, group_id=group.id, config_id=obj.id)
...
def extract_gis_data(obj, member, errored_files, success_files, arcgis_item_id):
    ...
    for layer_num, layer in enumerate(member.layers):
        ...
        try:
            simple_presentation = import_featuretype_presentation(
                layer.properties.drawingInfo.renderer, arc_item.arcgis_config
            )
    ...
    delete_result = (
        models.SpatialFeature.objects.filter(arcgis_item=arc_item).exclude(external_id__in=imported_global_ids).delete()
    )
...
def get_mb_style(symbol):
    ...
    if type == ESRI_LINE:
        ...
        if hasattr(symbol, "color") and symbol.color:
            ...
            presentation = {"stroke": colors_as_hex, "stroke-opacity": float(opacity)}
