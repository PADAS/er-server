...
def handle_gps_plus_position(position):
    logger.info(
        "GpsPlusPosition (%s) reported for collar: %s at %s, location: lon/lat %s, %s",
        position.id_position,
        position.id_collar,
        position.acquisition_time.isoformat(),
        position.longitude,
        position.latitude,
    )

    provider, created = SourceProvider.objects.get_or_create(provider_key=SOURCE_PROVIDER_KEY)
    ...
    source = Source.objects.ensure_source(
        ...
        subject={"subject_subtype_id": "unassigned", "name": manufacturer_id},
    )

    logger.debug(
        "{} source ({}) for collar_id: {}".format("Created" if created else "Found", source.id, position.id_collar)
    )
    ...
    additional = dict(
        ...
        and v is not None
        and k not in ("id_collar", "latitude", "longitude", "acquisition_time")
    )
    ...
    recorded_at = position.acquisition_time.replace(tzinfo=datetime.timezone.utc)
    observation = Obs(
        source=source, recorded_at=recorded_at, latitude=latitude, longitude=longitude, additional=additional
    )
    ...
    try:
        ...
        logger.info(
            "Recorded observation for collar_id: %s, at %s, longitude: %s, latitude: %s",
            position.id_collar,
        ...
        logger.exception(
            "Failed observation for collar_id: %s, at %s, longitude: %s, latitude: %s",
            position.id_collar,
            recorded_at.isoformat(),
            position.longitude,
            position.latitude,
