...
class IconKeyInput(Widget):
    ...
    def get_context(self, name, value, attrs):
        ...
        if context["widget"]["value"]:
            try:
                context["widget"]["file_path"] = next(o for o in image_list if o["key"] == context["widget"]["value"])[
                    "file_path"
                ]
...
def get_icon_select_list(dirname="sprite-src"):
    icon_list = [
        {"key": item.split(".")[0], "file_path": staticfiles_storage.url(os.sep.join((dirname, item)))}
        for item in staticfiles_storage.listdir(dirname)[1]
        if item.endswith(".svg") and item.count(".") == 1
    ]
