...
class GenericRelatedField(serializers.RelatedField):
    def get_field_mapping(self, label=None):
        ...
        default_mapping = {
            ...
            "activity.community": {"serializer": CommunitySerializer, "field": "community"},
        }
    ...
    def to_representation(self, value):
        ...
        mapping = field_mapping.get(value._meta.label_lower, None)
        if not mapping:
    ...
    def to_internal_value(self, data):
        ...
        return mapping["serializer"](read_only=True).to_internal_value(data)
    ...
    def object_choices(self):
        ...
        for provenance, values in queryset:
            choices += [(self.to_representation(item), self.display_value(item)) for item in values]
        return choices
