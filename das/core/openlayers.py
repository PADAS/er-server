    ...
    def map_options(self):
        ...
        for param_name, js_name, option_type in map_types:
            if self.params.get(param_name, False):
                if option_type == "srid":
                    value = ol_projection(self.params[param_name], self.params["units"])
...
class PropsOSMGeoAdminMixin(TileLayersMixin, admin.OSMGeoAdmin, SaveCoordinatesToCookieMixin):
    ...
    def get_map_widget(self, db_field):
        ...
        OLMap.params["tile_layers"] = self._get_tile_layers()
        return OLMap
...
class OSMGeoExtendedAdmin(PropsOSMGeoAdminMixin, SaveCoordinatesToCookieMixin):
    ...
    def response_add(self, request, obj, post_url_continue=None):
        if "_addanother" in request.POST:
            http_response = super().response_add(request, obj, post_url_continue)
            response = self.set_coordinates_cookie(http_response, obj)
            return response
