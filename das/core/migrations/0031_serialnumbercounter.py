...
def populate_counters(apps, schema_editor):
    ...
    # Clear any thread-local tenant so the tenant-aware managers on Event/Patrol
    # short-circuit instead of trying to read ``tenant_field`` from the historical
    # model (which does not carry ``TenantModelMixin``).
