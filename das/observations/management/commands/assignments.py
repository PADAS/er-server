...
def update_assignment(assignment, start_date=None, end_date=None, force=False):
    ...
    new_assignment_range = psycopg2.extras.DateTimeTZRange(lower=start_date, upper=end_date)

    SubjectSource.objects.filter(id=assignment.id).update(assigned_range=new_assignment_range)
...
def add_assignment(subject, source, start_date=None, end_date=None):
    ...
    conflicting_assignments = SubjectSource.objects.filter(
        Q(subject=subject) | Q(source=source), assigned_range__overlap=assignment_range
    )
    ...
    if conflicting_assignments:
        ...
        SubjectSource.objects.create(subject=subject, source=source, assigned_range=assignment_range)
...
def ensure_assignment(subject, manufacturer_id, model_name, source_type, source_provider_key, start_date):
    ...
    if srcs:
        ...
        source = Source.objects.create(
            manufacturer_id=manufacturer_id, model_name=model_name, provider=source_provider, source_type=source_type
        )

    if subject and source:
        existing_assignment = find_assignments(subject=subject, source=source, start_date=start_date)
