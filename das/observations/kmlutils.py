...
def render_to_kmz(content, filename):
    ...
    response = Response(zip_io.getvalue(), content_type="application/vnd.google-earth.kmz")
...
def get_kml_access_token(user, ttl=KML_TOKEN_TTL_DAYS):
    try:
        ...
        logger.error("There exists no Oauth2 Application with client_id %s", KML_OAUTH_CLIENT_ID)
...
def get_kml_master_link(user, request):
    ...
    return utils.add_base_url(request, "?".join((reverse("subjects-kml-root-view"), "auth={}".format(token))))
