...
class PatrolsMaterializedView:
    ...
    def generate_ddl(self):
        ...
        CREATE MATERIALIZED VIEW IF NOT EXISTS {self.table_name} AS
            ...
            (SELECT to_four_dps(SUM(patrol_distance)) as "Distance covered (km)" FROM (SELECT
                ...
                FROM (SELECT serial_number, location FROM observations_subjectstatus WHERE subject_id=ps.leader_id) prev JOIN
                    observations_subjectstatus curr ON prev.serial_number = curr.serial_number - 1 WHERE curr.serial_number >= 1) AS "distances"),
    ...
    def check_view_exists(self):
        cursor = self.cursor()
        cursor.execute("SELECT to_regclass('public.{0}')".format(self.table_name))
