...
def test_observation_signal_invalidation(monkeypatch, fake_vector_tile_cache):
    ...
    deleted = _invalidate_for_point(tenant_id=tenant_id, layer_ids=(layers_part,), lon=lon, lat=lat)
    assert deleted >= 2
...
def test_segment_signal_invalidation(monkeypatch, fake_vector_tile_cache):
    ...
    count_a = _invalidate_for_point(tenant_id=str(tenant.id), layer_ids=(layers_part,), lon=12.5, lat=-1.25)
    count_b = _invalidate_for_point(tenant_id=str(tenant.id), layer_ids=(layers_part,), lon=12.51, lat=-1.26)
