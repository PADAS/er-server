...
class TestSubjectSourceSerializer:
    ...
    def test_subject_source_serializer_no_stationary_subject(self, subject_source):
        ...
        assert subject_source_serialized["assigned_range"]["lower"] == subject_source.assigned_range.lower.astimezone(
        ).isoformat()
        assert subject_source_serialized["assigned_range"]["upper"] == subject_source.assigned_range.upper.astimezone(
        ).isoformat()
    ...
    def test_subject_source_serializer_for_stationary_subject(self, subject_source):
        ...
        assert subject_source_serialized["id"] == str(subject_source.id)
        assert subject_source_serialized["assigned_range"][
            "lower"] == subject_source.assigned_range.lower.astimezone().isoformat()
        assert subject_source_serialized["assigned_range"][
            "upper"] == subject_source.assigned_range.upper.astimezone().isoformat()
        assert subject_source_serialized["subject"] == subject_source.subject.id
        assert subject_source_serialized["source"] == subject_source.source.id
        assert subject_source_serialized["location"] == {"latitude": str(
            subject_source.location.y), "longitude": str(subject_source.location.x)}
        assert additional["comments"] == subject_source.additional.get(
            "comments")
        assert additional["chronofile"] == subject_source.additional.get(
            "chronofile")
