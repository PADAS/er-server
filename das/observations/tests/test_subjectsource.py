...
def generate_observation_data(source_id):
    ...
    observation = {"location": location, "recorded_at": observation_time, "source": source_id, "additional": {}}
    serializer = ObservationSerializer(data=observation)
    if serializer.is_valid():
        observation = serializer.save()

    subject_statuses = SubjectStatus.objects.filter(subject__subjectsource__source_id=source_id, delay_hours=0)
...
class SubjectSourceTestCase(BaseAPITest):
    def setUp(self):
        ...
        self.user = User.objects.create_user(
            "user", "user@test.com", "all_perms_user", is_superuser=True, is_staff=True, **self.user_const
        )
    ...
    def test_subjectsource_with_empty_assignedrange(self):
        ...
        subject, created = Subject.objects.get_or_create(name="Assigned Subject")
        provider, created = SourceProvider.objects.get_or_create(provider_key="assignment-test-provider")
        source, created = Source.objects.get_source(manufacturer_id="assignment-test-01", provider=provider)

        ss = SubjectSource.objects.create(subject=subject, source=source, assigned_range="empty")
        ...
        SubjectSource.objects.filter(id=ss.id).update(assigned_range=DEFAULT_ASSIGNED_RANGE)
    ...
    def test_update_source(self):
        ...
        subject_status, longitude, latitude = generate_observation_data(source_id=source_id)
        self.assertEqual((subject_status.location.x, subject_status.location.y), (longitude, latitude))

        subject_status, longitude, latitude = generate_observation_data(source_id=source_id2)
        self.assertEqual((subject_status.location.x, subject_status.location.y), (longitude, latitude))
    ...
    def test_parse_comma_function(self):
        ...
        self.assertEqual(
            listed_source_id,
            [uuid.UUID("0d9725c0-c186-464f-98f4-a45d31f81efd"), uuid.UUID("0a308294-7b80-4633-a967-ef4f8e1de79a")],
        )
    ...
    def test_sources_api(self):
        ...
        source, _ = Source.objects.get_or_create(
            id=uuid.UUID("1f199c72-7a52-4659-be86-4ac40231826f"),
            manufacturer_id="#01-manufacurer_id",
            provider=provider,
        )

        source2, _ = Source.objects.get_or_create(
            id=uuid.UUID("7cbbd57e-0026-46a2-9726-32849a527326"),
            manufacturer_id="#02-manufacurer_id",
            provider=provider,
        )
    ...
    def test_subjectsources_api(self):
        ...
        ss2 = SubjectSource.objects.create(
            subject=subject, source=source2, assigned_range=DateTimeTZRange(lower=DEFAULT_ASSIGNED_RANGE[0])
        )

        # asign subject (#02-subject) only one source.
        ss3 = SubjectSource.objects.create(
            subject=subject2, source=source3, assigned_range=DateTimeTZRange(lower=DEFAULT_ASSIGNED_RANGE[0])
        )
...
class TestSourceAPITestCase:
    def test_create_source_api(self, superuser_client):
        ...
        provider, _ = SourceProvider.objects.get_or_create(provider_key=provider_key)
        source_data = dict(manufacturer_id=faker.last_name(), provider=provider_key, additional={})
        url = reverse("sources-view")

        response = superuser_client.post(url, data=source_data)

        assert subject_count == Subject.objects.count()
        assert response.status_code == status.HTTP_201_CREATED
        unset_current_tenant()

    def test_delete_source_api(self, superuser_client):
        set_current_tenant(self._get_tenant())
        faker = Faker()
        provider_key = f"{faker.last_name()}_{faker.last_name()}"
        provider, _ = SourceProvider.objects.get_or_create(provider_key=provider_key)
    ...
    def test_delete_observation_api(self, superuser_client):
        ...
        lob = LatestObservationSource.objects.filter(source_id=source_id).first()

        urlpath = reverse("observation-view", kwargs={"id": lob.observation.id})
