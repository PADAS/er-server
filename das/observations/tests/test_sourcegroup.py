...
class AdditionalTestCase(BaseAPITest):
    ...
    def test_source_group(self):
        ...
        self.assertTrue(any(str(self.henry.id) == subject.get("id") for subject in response.data))

        # Check accessibility of observations for subject
        track_request = self.factory.get(self.api_base + "/{}/tracks/".format(self.henry.id))
        self.force_authenticate(track_request, self.satellite_user)

        response = views.SubjectTracksView.as_view()(track_request, subject_id=self.henry.id)
        self.assertEqual(response.status_code, 200)
        ...
        observations = [
            obs.location.coords for obs in Observation.objects.filter(source__manufacturer_id="junk-fix-source")
        ]
    ...
    def test_multiple_source_observations_for_single_subject(self):
        ...
        response = views.SubjectsView.as_view()(request)
        self.assertEqual(response.status_code, 200)
        self.assertTrue(any(str(self.junkie.id) == subject.get("id") for subject in response.data))
        ...
        request = self.factory.get(self.api_base + "/{}/tracks/".format(self.junkie.id))
        self.force_authenticate(request, self.satellite_user)
        response = views.SubjectTracksView.as_view()(request, subject_id=self.henry.id)
        self.assertEqual(response.status_code, 200)
        coordinates = response.data["features"][0]["geometry"]["coordinates"]
        observations = [obs.location.coords for obs in Observation.objects.filter(source__manufacturer_id="awt")]
        self.assertFalse(any(obs in coordinates for obs in observations))

        # Junkie can't access all observations linked with junk-fix-source
        # It was linked for some time period and than de-linked
        observations_count = Observation.objects.filter(source__manufacturer_id="junk-fix-source").count()
    ...
    def test_source_not_linked_with_any_source_group(self):
        ...
        for user in [self.gps_user, self.satellite_user]:
            ...
            for subject in response.data:
                track_request = self.factory.get(self.api_base + "/{}/tracks/".format(subject.get("id")))
                self.force_authenticate(track_request, user)
                observations = views.SubjectTracksView.as_view()(request, subject_id=subject.get("id"))
                observations = observations.data["features"][0]["geometry"]["coordinates"]
                coordinates.append(observations)
        awt_observations = [obs.location.coords for obs in Observation.objects.filter(source__manufacturer_id="awt")]
