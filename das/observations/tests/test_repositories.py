...
def test_get_observations_instance(observation, source) -> None:
    repository = ReadObservationRepository(source)
...
@pytest.mark.django_db
@pytest.mark.parametrize("repo_source", ["database"])
def test_get_observations_by_subject_id_and_source_id(subject_source_with_observations, repo_source):
    ...
    repository = ReadObservationRepository(repo_source)
