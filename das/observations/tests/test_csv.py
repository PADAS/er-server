...
class TrackingMetaDataExportViewTest(BaseAPITest):
    ...
    def test_csv_metadata_with_simple_user(self):
        ...
        subject_names = [subject.name for subject in self.subject_group.get_all_subjects(self.user)]
        ...
        chronofiles = (474, 256)
        chronofiles_in_meta = [int(metadata["chronofile"]) for metadata in metadatas]
...
class TrackingDataCsvViewTest(BaseAPITest):
    ...
    def test_csv_observation_data(self):
        ...
        self.assertEqual(
            Observation.objects.filter(
                source__subjectsource__assigned_range__contains=F("recorded_at"), exclusion_flags=0
            ).count(),
            len(csv_data),
        )

    def filter_observations_with_exclusion_flag(self, observation_filter):
        self.request = self.factory.get(API_BASE + "/trackingdata/export/?{0}".format(urlencode(observation_filter)))
        self.force_authenticate(self.request, self.superuser)
        response = TrackingDataCsvView.as_view()(self.request)
        self.assertEqual(response.status_code, 200)
        csv_data = response.content.decode("utf-8").split("\r\n")
    ...
    def test_csv_observation_data_with_exclusion_flag(self):
        ...
        self.assertEqual(
            Observation.objects.filter(
                source__subjectsource__assigned_range__contains=F("recorded_at"), exclusion_flags=1
            ).count(),
            len(csv_data),
        )
    ...
    def test_csv_observation_data_with_exclusion_flag_set_to_null(self):
        ...
        self.assertEqual(
            Observation.objects.filter(source__subjectsource__assigned_range__contains=F("recorded_at")).count(),
            len(csv_data),
        )
    ...
    def test_normal_user_access_subject_observation_data(self):
        ...
        "dloadtime ({})".format(tz_offset)
        for observation in observations:
            fixtime_key = next(key for key in observation.keys() if key.startswith("fixtime"))
            next(key for key in observation.keys() if key.startswith("dloadtime"))
            break
    ...
    def exportrecords(self, url):
        self.subject_group.permission_sets.add(PermissionSet.objects.get(name="View Tracks All Time"))
        self.user.permission_sets.add(PermissionSet.objects.get(name="View Tracks All Time"))
    ...
    def test_different_chronofile_values_for_same_subject(self):
        ...
        chrono_files = [observation["chronofile"] for observation in observations]
    ...
    def test_tracking_data_for_specific_subject(self):
        ...
        unique_subject_ids = list(set([observation["subject_id"] for observation in observations]))
    ...
    def test_tracking_data_current_status_for_specific_subject(self):
        request = self.factory.get(
            API_BASE
            + "/trackingdata/export/?current_status=true&format=json&subject_id=0fa8ec9a-7e92-4575-9575-df202d5dde25?"
        )
    ...
    def test_csv_observation_with_inactive_subject(self):
        ...
        africa_subject_group = SubjectGroup.objects.get(name="African elephant subjet group")
        africa_subject_group.permission_sets.add(PermissionSet.objects.get(name="View Tracks All Time"))
        ...
        csv_observation_ids = [observation[fixtime_key] for observation in observations]

        observation_filter = {"filter": 1}
        request = self.factory.get(API_BASE + "/trackingdata/export/?{0}".format(urlencode(observation_filter)))
        self.force_authenticate(request, self.user)
        response = TrackingDataCsvView.as_view()(request)
        self.assertEqual(response.status_code, 200)
        csv_data = response.content.decode("utf-8").split("\r\n")
        ...
        self.assertTrue(
            any(
                [
                    obs_time.astimezone(current_tz) in unique_csv_observation_ids
