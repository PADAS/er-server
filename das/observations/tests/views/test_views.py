...
from factories import SubjectFactory, SubjectSubTypeFactory
...
def _subject_ids(response) -> set[str]:
    """Extract Subject IDs from a subjects-list-view response."""
    data = response.data
...
@pytest.mark.django_db
@pytest.mark.usefixtures("tenant_settings", "das_tenant_monkeypatch")
class TestSubjectsViewFilters:
    """Test new filter query params on the subjects-list-view endpoint."""

    base_url = "subjects-list-view"

    @pytest.fixture()
    def filter_dataset(self):
        sst_rhino = SubjectSubTypeFactory.create(value="rhino_svf")
        sst_ranger = SubjectSubTypeFactory.create(value="ranger_svf")

        cn_black = CommonName.objects.create(
            value="black_rhino_svf",
            display="Black Rhino",
            subject_subtype=sst_rhino,
        )
        cn_white = CommonName.objects.create(
            value="white_rhino_svf",
            display="White Rhino",
            subject_subtype=sst_rhino,
        )
        ...
        s["br_calf"] = SubjectFactory.create(
            subject_subtype=sst_rhino,
            common_name=cn_black,
            additional={"sex": "male", "age": "calf"},
        )
        s["wr"] = SubjectFactory.create(
            subject_subtype=sst_rhino,
            common_name=cn_white,
        )
        s["ranger"] = SubjectFactory.create(subject_subtype=sst_ranger)

        grp = SubjectGroup.objects.create(name="FieldTeam_svf")
        grp.subjects.add(s["ranger"])

        return s

    # -- common_name --

    def test_filter_by_common_name(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?common_name=black_rhino_svf")

        assert response.status_code == status.HTTP_200_OK
        ids = _subject_ids(response)
        assert str(s["br_male"].id) in ids
        assert str(s["br_female"].id) in ids
        assert str(s["wr"].id) not in ids
        assert str(s["ranger"].id) not in ids

    def test_filter_by_multiple_common_names(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?common_name=black_rhino_svf,white_rhino_svf")

        assert response.status_code == status.HTTP_200_OK
        ids = _subject_ids(response)
        assert str(s["br_male"].id) in ids
        assert str(s["wr"].id) in ids
        assert str(s["ranger"].id) not in ids

    # -- common_name_search --

    def test_filter_by_common_name_search(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?common_name_search=rhino_svf")
    ...
    def test_filter_by_group_name(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?group_name=FieldTeam_svf")

        assert response.status_code == status.HTTP_200_OK
        ids = _subject_ids(response)
        assert str(s["ranger"].id) in ids
        assert str(s["br_male"].id) not in ids

    # -- additional --

    def test_filter_by_additional_key(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?additional__sex=male")

        assert response.status_code == status.HTTP_200_OK
        ids = _subject_ids(response)
        assert str(s["br_male"].id) in ids
        assert str(s["br_female"].id) not in ids

    def test_filter_by_multiple_additional_keys(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?additional__sex=male&additional__age=adult")

        assert response.status_code == status.HTTP_200_OK
        ids = _subject_ids(response)
        assert str(s["br_male"].id) in ids
        assert str(s["br_calf"].id) not in ids
        assert str(s["br_female"].id) not in ids

    # -- composition --

    def test_common_name_combined_with_additional(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?common_name=black_rhino_svf&additional__sex=female")
    ...
    def test_subtypes_combined_with_group_name(self, superuser_client, filter_dataset):
        s = filter_dataset
        url = reverse(self.base_url)
        response = superuser_client.get(f"{url}?subject_subtypes=ranger_svf&group_name=FieldTeam_svf")
