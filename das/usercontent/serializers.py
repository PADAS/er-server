...
def resolve_file_icon(filecontent):
    try:
        ...
        image_key = None
    return (
        static_image_finder.get_marker_icon(
            [
                image_key,
            ]
        )
...
class FileContentSerializer(rest_framework.serializers.ModelSerializer):
    ...
    created_by = rest_framework.serializers.HiddenField(default=rest_framework.serializers.CurrentUserDefault())

    file = rest_framework.serializers.FileField(write_only=True)
