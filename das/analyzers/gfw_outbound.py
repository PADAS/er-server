...
NETWORK_ERROR_MESSAGE = _(
    "Network error occurred while setting up your subscription. Please try again later. For further assistance contact Support."
)
GFW_ERROR_MESSAGE = _(
    "While setting up your subscription, GFW responded with the following error. For further assistance contact Support. GFW error:"
)
...
def get_webhook_base_url(provider_key=GFWAlertHandler.PROVIDER_KEY):
    ...
    :return:
    """
    path = reverse("gfahandler-view", kwargs={"provider_key": provider_key})
...
def get_gfw_oauth2_application():
    app, created = Application.objects.get_or_create(
        client_id=GFW_OAUTH_APPLICATION_ID,
        defaults={
...
def get_gfw_access_token(user, ttl_days=5 * 365):
    ...
    try:
        ...
        logger.error("There exists no Oauth2 Application with client_id %s", GFW_OAUTH_APPLICATION_ID)
    else:
        try:
    ...
    if gfw_credentials:
        try:
            login_response = requests.post(
                url=login_endpoint,
                json={"email": gfw_credentials.get("username"), "password": gfw_credentials.get("password")},
            )
...
def create_subscription(gfw_info):
    ...
    if token:
        ...
        subscribe_json = _make_subscribe_msg(gfw_info["name"], gfw_info["alert_types"], geostore_id)
        ...
        try:
            ...
            if rsp and rsp.status_code == status.HTTP_200_OK:
                ...
                return _make_service_response(
                    rsp.status_code, "Success", dict(subscription_id=sub_id, geostore_id=geostore_id)
                )
            else:
    ...
    if token:
        try:
            ...
            if rsp and rsp.status_code == status.HTTP_200_OK:
                ...
                _make_service_response(rsp.status_code, "Success", dict(json=json.loads(rsp.text).get("data", {})))
            else:
...
def _get_geostore_id(gfw_info):
    ...
    try:
        rsp = requests.post(
            url=f"{settings.GFW_API_ROOT}/geostore", json=json_dict, timeout=DEFAULT_REQUESTS_TIMEOUT_SECS
        )
    except Exception as ex:
        ...
        if rsp and rsp.status_code == status.HTTP_200_OK:
            ...
            logger.error(
                f"_get_geostore_id failed. {rsp.status_code} {rsp.text} geostore geojson: {json.dumps(json_dict)}"
            )
            err_msg = f"{GFW_ERROR_MESSAGE} {_get_error_detail_as_string(rsp.text)}"
            return _make_service_response(rsp.status_code, _(err_msg))
...
def _make_service_response(status_code, status_text, data=None):
    return {"status_code": status_code, "text": status_text, "data": data}
...
def exception_wrapper(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            response = _make_service_response(
                SERVICE_ERROR_CODE, f"Error communicating with Global Forest Watch service {func} raised {ex}"
            )
