...
class SubjectProximityAnalyzer(ProximityAnalyzer):
    ...
    def analyze_trajectory(self, traj=None):
        ...
        traj = pymet.base.Trajectory(
            relocs=pymet.base.Relocations(fixes=traj.relocs.get_fixes()[-2:], subject_id=self.subject.id)
        )
        ...
        for prox in proximity_results.proximity_events:
            ...
            if prox.proximity_distance_meters <= self.config.threshold_dist_meters:
                ...
                result = SubjectAnalyzerResult(
                    ...
                    title=self.subject.name + str(_(" is near ")) + subject_2_name + ".",
                    level=CRITICAL,
                    message=self.subject.name + str(_(" is near ")) + subject_2_name + ".",
                    analyzer_revision=1,
...
class SubjectProximityAnalysis:
    ...
    def verify_proximal_tracks_time_frame(
        cls, config, latest_observation_analysis_subject, latest_observation_second_subject
    ):
        if (
            latest_observation_analysis_subject
            and latest_observation_second_subject
            and abs(
                latest_observation_analysis_subject.recorded_at - latest_observation_second_subject.recorded_at
            ).total_seconds()
            <= config.proximity_time * 3600
        ):
            return True
            ...
            for seg in traj.traj_segs:
                for subject in proximity_analysis_params:
                    ...
                    subject_trajectories = pymet.base.Trajectory(
                        relocs=pymet.base.Relocations(fixes=subject_traj.relocs.get_fixes()[-2:], subject_id=subject.id)
                    )
...
class SubjectProximityEvent:
    ...
    def __init__(
        ...
        subject_2_speed,
        subject_2_location,
        subject_1_travel_heading=0.0,
        subject_2_travel_heading=0.0,
        proximal_fix=None,
...
SUBJECT_PROXIMITY_SCHEMA = {
    ...
    "definition": [
        {"type": "fieldset", "title": "Analyzer Details", "htmlClass": "col-lg-12", "items": ["analyzer_name"]},
        {
            "type": "fieldset",
            "htmlClass": "col-lg-6",
            "items": ["subject_1_name", "subject_1_location", "subject_1_speed_kmhr", "subject_1_heading"],
        },
        {
            "type": "fieldset",
            "htmlClass": "col-lg-6",
            "items": ["subject_2_name", "subject_2_location", "subject_2_speed_kmhr", "subject_2_heading"],
        },
