...
from analyzers.gfw_alert_schema import GFW_EVENT_TYPES_MAP, ensure_gfw_event_types
from analyzers.gfw_utils import (
    prepare_downloadable_url,
...
def process_handler_post(request):
    ...
    if not deserialized.is_valid():
        ...
        log_metrics({PROCESSED_COUNTER: 0, ERROR_COUNTER: 1})
        return Response(status=status.HTTP_400_BAD_REQUEST, data=deserialized.errors)

    logger.info(f"process_handler_alerts posted {deserialized.validated_data}", extra={"data": request.data})
    ...
    if not GFW_EVENT_TYPES_MAP.get(layer_slug):
        ...
        return Response(status=status.HTTP_400_BAD_REQUEST, data=dict(message=f"Unknown layerSlug: {layer_slug}"))
    ...
    if subscriptions_qs.exists():
        ...
        logger.warning(
            "Unknown subscription %s received. Processing alerts for all subscriptions in db.", subscription_id
        )
        subscription_ids = [
            o.subscription_id
            for o in GlobalForestWatchSubscription.objects.all()
...
def process_alert_for_subscription(layer_slug, subscription_id, validated_data, user_id, polling=False):
    ...
    if event_dict.get("event_type") == "gfw_glad_alert" and not polling:
        ...
        download_url = rebuild_glad_download_url(
...
def process_downloaded_alerts(payload, common_event_fields, user_id):
    ...
    errors = [
        create_event_from_downloadedalert(alert, common_event_fields, user_id, counts) for alert in clustered_alerts
    ]
...
def create_event_from_downloadedalert(downloaded_sample, common_event_fields, user_id, counts):
    ...
    common_event_fields["event_details"]["num_clustered_alerts"] = num_clustered_alerts
    ...
    for alert in alerts:
        try:
            ...
            if common_event_fields.get("event_type") == "gfw_activefire_alert":
                ...
                superset_confidence = {i.strip() for i in conf_confidence.split(",")}
            else:
                conf_confidence = gfw_query.Deforestation_confidence
                superset_confidence = {int(i) for i in conf_confidence.split(",")}
...
def persist_event(event_fields, request, counts):
    ...
    qs = EventDetails.objects.filter(
    ...
    if qs.exists():
        ...
        signals.pre_save.connect(pre_save_info, dispatch_uid=(__name__, request, event_fields), weak=False)
