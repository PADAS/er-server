...
def require_earthengine(func):
    def f1(self, *args, **kwargs):
        ...
        try:
            ...
            msg = f"Unable to load Earth Engine configuration. Error: {jex}"
            logger.warning(msg)
            raise ValueError(msg)
        except Exception:
    ...
    "schema": {
        ...
        "properties": {
            ...
            "environmental_descriptor": {
                "type": "string",
                "title": "Environmental Descriptor",
            },
            "mean_value": {
                "type": "number",
                "title": "Mean Value",
            ...
            "img_name": {
                "type": "string",
                "title": "Earth Engine Image Name",
            },
            "img_band_name": {
                "type": "string",
                "title": "Image Band Name",
            },
...
def ensure_environmental_event_types():
    ec, created = EventCategory.objects.get_or_create(value="analyzer_event", defaults=dict(display="Analyzer Events"))

    for et in [EnvironmentalValueEventType, EnvironmentalAllClearEventType]:
        EventType.objects.get_or_create(
            value=et.value,
            category=ec,
            defaults=dict(display=et.display, schema=json.dumps(et.schema, indent=2, default=str)),
        )
...
class EnvironmentalAnalyzer(SubjectAnalyzer):
    ...
    def get_subject_analyzers(cls, subject):
        ...
        for ac in EnvironmentalSubjectAnalyzerConfig.objects.filter(subject_group__in=subject_groups, is_active=True):
    ...
    def analyze_trajectory(self, traj=None):
        ...
        result = SubjectAnalyzerResult(
            ...
            level=OK,
            title=self.subject.name + str(_(": low " + self.config.short_description)),
            message=self.subject.name + str(_(" is in a low " + self.config.short_description + " area.")),
            analyzer_revision=1,
        ...
        mean_value = eetools.extract_point_values_from_image(
            relocs=traj.relocs,
        ...
        if mean_value is not None:
            ...
            if mean_value > self.config.threshold_value:
                ...
                result.title = self.subject.name + str(_(": high " + self.config.short_description))
                result.message = self.subject.name + str(_(" is in a high " + self.config.short_description + " area."))
        return [
            result,
        ]
