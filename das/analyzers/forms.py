...
class TimeFrameField(forms.fields.MultiValueField):
    ...
    error_message_minutes = {
        "max_value": "Ensure 'value for minutes' is less than or equal to 59",
...
class EnvironmentalAnalyzerAdminForm(JSONFieldFormMixin, forms.ModelForm):
    ...
    earth_engine_json_key = forms.CharField(
        label="Earth Engine JSON Key",
        widget=FixedWidthFontTextArea(attrs={"cols": "100", "rows": "30"}),
        required=False,
        help_text=_("Paste the contents of your Earth Engine JSON key here."),
    )
...
class GlobalForestWatchSubscriptionForm(JSONFieldFormMixin, forms.ModelForm):
    ...
    glad_confirmed_backfill_days = forms.IntegerField(
        initial=180,
        max_value=365,
        min_value=10,
