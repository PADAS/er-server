...
class GFWServiceTest(BaseAPITest):
    ...
    def test_create_geostore(self):
        ...
        self.assertEqual(id, "b52eafedb512d22390b50fabfed0a18a")
    ...
    def test_update_subscription(self):
        ...
        rsp = self._get_data(dest_url=f"{SUBSCRIPTION_ENDPOINT}/{updated_sub_id}", headers=AUTH_HEADER)

        self.assertIsNotNone(rsp)
        self.assertEqual(rsp.status_code, status.HTTP_200_OK)
        rsp_payload = json.loads(rsp.text)["data"]
        # datasets should have been updated
        self.assertEqual(gfw_test_data.FIRE_ALERT_SUBSCRIPTION_DATA["datasets"], rsp_payload["attributes"]["datasets"])
    ...
    def _create_and_verify_subscription(self, gfw_info):
        ...
        rsp = self._get_data(dest_url=f"{SUBSCRIPTION_ENDPOINT}/{sub_id}", headers=AUTH_HEADER)
