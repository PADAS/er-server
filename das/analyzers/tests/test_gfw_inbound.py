...
from analyzers.tasks import (
    download_gfw_alerts,  # prevent pycharm optimize import from removing this
)
from analyzers.tests.gfw_test_data import (
    GLAD_ALERT,
...
class GFWAlertHandlerTest(BaseAPITest):
    ...
    def test_glad(self, mock_request):
        ...
        self._create_and_get_test_model(
            subscription_id=self.test_data_glad_subscription_id, geostore_id=self.test_data_geostore_id
        )
    ...
    def test_rebuild_glad_url_confirmed_only(self):
        ...
        updated_url = rebuild_glad_download_url(self.download_url_unknown_geostore, gfw_obj)
    ...
    def test_filter_confidence_level_for_deforestation(self, mock_request):
        ...
        response = self._post_data(json.dumps(GLAD_ALERT))
        clustered_alerts = cluster_alerts(GLAD_ALERT_DOWNLOADED_DATA["data"], settings.GFW_CLUSTER_RADIUS, 1)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(clustered_alerts), Event.objects.all().count())
    ...
    def test_backfill_scheduling(self, mock_date):
        ...
        for i in range(6):
            self._verify_alert_info(
                subscription, alert_infos[i + 1], interval_start, interval_start + timedelta(days=30), True
            )
    ...
    def _verify_alert_info(
        self,
        subscription: gfw_model,
        alert_info: dict,
        ...
        self._verify_download_url(
            subscription,
            alert_info["downloadUrls"]["json"],
            expected_start_date,
            expected_end_date,
            expected_confirmed_only,
        )

    def _verify_download_url(
        self,
        subscription: gfw_model,
        download_url: str,
        expected_start_date: date,
        expected_end_date: date,
        expected_confirmed_only: bool,
    ):
    ...
    def _verify_viirs_params(self, subscription: gfw_model, start_date: date, end_date: date, virrs_params: dict):
    ...
    def _create_and_get_test_model(
        self, subscription_id=None, geostore_id=None, glad_conf=None, viirs_conf=None, additional=None
    ):
        ...
        return gfw_model.objects.create(
            name="Test alert",
