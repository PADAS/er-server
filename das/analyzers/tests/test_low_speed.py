...
from analyzers.tests.low_speed_test_data import HERITAGE_Track
...
class TestLowSpeedAnalyzer(TestCase):
    ...
    def low_speed_percentile_event_schema_json(cls):
        schema_yaml = """
                schema:
                  ...
                  type: object
                """
        this_json = json.dumps(yaml.load(schema_yaml, Loader=yaml.FullLoader))
        print(this_json)
        return this_json
    ...
    def low_speed_percentile_all_clear_event_schema_json(cls):
        schema_yaml = """
                    schema:
                      ...
                      type: object
                    """
        return json.dumps(yaml.load(schema_yaml, Loader=yaml.FullLoader))
    ...
    def setUp(self):
        ...
        EventType.objects.get_or_create(
            ...
            category=ec,
            defaults=dict(
                display="Low Speed Percentile Analyzer", schema=self.low_speed_percentile_event_schema_json()
            ),
        )
        ...
        EventType.objects.get_or_create(
            ...
            category=ec,
            defaults=dict(
                display="Low Speed Percentile Analyzer All Clear",
        ...
        EventType.objects.get_or_create(
            ...
            category=ec,
            defaults=dict(display="Low Speed Wilcox Analyzer", schema=self.low_speed_wilcox_event_schema_json()),
        )
        ...
        EventType.objects.get_or_create(
            ...
            category=ec,
            defaults=dict(
                display="Low Speed Wilcox Analyzer All Clear",
    ...
    def test_low_speed_analyzer(self):
        ...
        for result in results:
            print(f"Low Speed Result: {result}")
    ...
    def test_wilcoxon_result(self):
        ...
        self.assertTrue(len(results) > 0)
        for result in results:
            print(f"Low Speed Wilcox Result: {result}")
