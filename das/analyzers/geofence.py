...
class GeofenceAnalyzer(SubjectAnalyzer):
    ...
    def get_subject_analyzers(cls, subject):
        ...
        for ac in GeofenceAnalyzerConfig.objects.filter(subject_group__in=subject_groups, is_active=True):
    ...
    def analyze_trajectory(self, traj=None):
        ...
        for cross in cross_results.geofence_crossings:
            ...
            result.geometry_collection = DjangoGeoColl(
                [
                    DjangoPoint(
                        cross.est_cross_fix.geopoint.ogr_geometry.GetX(),
            ...
            contain_names = ",".join(
                [SpatialFeature.objects.get(pk=contain_id).name for contain_id in cross.end_region_ids]
            )
