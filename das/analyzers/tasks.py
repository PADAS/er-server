...
def _analyze_subject(subject_id):
    ...
    try:
        ...
        logger.warning("No active Subject found by ID in analyze_subject. id=%s", subject_id)
    ...
    if subject:
        ...
        for analyzer in get_subject_analyzers(subject):
            ...
            if analyzer_key and cache.get(analyzer_key):
                logger.info(f"The analyzer {analyzer.config.id} is quiet for a while")
                continue
...
def fetch_alerts(self, event_dict, download_url, user_id):
    try:
        ...
        if event_dict.get("event_type") == GFWGladEventTypeSpec.value:
            logger.info(
                "Processing GFW payload for %s. Downloading from: %s", event_dict.get("event_type"), download_url
            )
            ...
            logger.info(
                "Processing GFW payload for %s. Downloading from query params: %s",
        ...
        logger.exception("Failed downloading GFW alert data for url: %s", download_url, extra={"Exception": tex})
        self.retry(countdown=60)
        result = f"Failure: {tex}"
    except Exception as ex:
        logger.exception("Failed downloading GFW alert data for url: %s", download_url, extra={"Exception": ex})
        result = f"Failure: {ex}"
    else:
