...
NO_LOCATION_SAMPLES = [
    {"EXIF": """""", "provider_name": "jenga", "image_name": "2019-05-20_151152.jpg"},
]
...
class CameraTrapTest(BaseAPITest):
    ...
    def setUp(self):
        ...
        self.sensor_user = User.objects.create_user(
            "sensor_user", "sensor_user@test.com", "sensoruser", **self.user_const
        )
    ...
    def post_cam_image(self, sample, data=None):
        ...
        if not data:
            data = {}
        data.update({"filecontent.file": SimpleUploadedFile(filename, f.read(), content_type="image/jpg")})
    ...
    def test_post_image_with_data(self):
        data = {
            "camera_name": "With Data",
            "location": json.dumps({"latitude": -2.08187, "longitude": 34.49477}),
    ...
    def test_post_image_to_previous(self):
        ...
        for sample in SAMPLES:
            if group_id:
                response = self.post_cam_image(sample, data={"group_id": group_id})
            else:
                response = self.post_cam_image(sample)
            self.assertEqual(response.status_code, 201)
    ...
    def test_default_priority_urgent(self):
        self.assertEqual(camera_trap.get_priority(), camera_trap.Event.PRI_URGENT)
    ...
    def test_default_priority_loaded_from_settings(self):
        for priority in camera_trap.Event.PRIORITY_CHOICES:
            ...
            with self.settings(SENSORS=camera_settings):
                self.assertEqual(camera_trap.get_priority(), priority)
