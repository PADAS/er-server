...
class FollowltObservationTest(BaseAPITest):
    ...
    user_const = dict(last_name="Lastname", first_name="Firstname", is_superuser=False)
    ...
    def setUp(self):
        ...
        super().setUp()
        self.testuser = User.objects.create_user("das_trbonet", "das@tempuri.org", "somesecret", **self.user_const)
        ...
        self.test_source = Source.objects.create(
            source_type="tracking-device", provider=self.test_sourceprovider, manufacturer_id="followlt-1234"
        )

        self.test_subjectsource = SubjectSource.objects.create(
            source=self.test_source, subject=self.henry, assigned_range=DEFAULT_ASSIGNED_RANGE
        )
    ...
    def test_post_new_radio_update(self):
        ...
        path = "/".join((self.api_base, "sensors", self.sensor_type, self.provider_key, "status"))
