...
from activity.serializers import EventFileSerializer, EventSerializer
...
class CameraTrapSensorHandler:
    ...
    def post_camera_trap_report(cls, request, params):
        ...
        if ".png" in file_name.lower():
            return Response(data="png is not a supported media type", status=status.HTTP_400_BAD_REQUEST)
        ...
        if params.validated_data["camera_name"]:
            ...
            camera_name = exif_dict["Model"].decode("utf-8") if exif_dict.get("Model") else None
        ...
        if params.validated_data["group_id"]:
            try:
                ...
                message = "Group with id {group_id} for image not found".format(params.validated_data["group_id"])
            ...
            if not eser.is_valid():
                return Response(data=eser.errors, status=status.HTTP_400_BAD_REQUEST)
            with transaction.atomic():
                event = eser.create(eser.validated_data)
                event_file_ser = EventFileSerializer(
                    data={"event": event.id, "file": file}, context={"request": request}
                )
                if not event_file_ser.is_valid():
                    return Response(data=event_file_ser.errors, status=status.HTTP_400_BAD_REQUEST)
                event_file_ser.create(event_file_ser.validated_data)

        return Response({"group_id": str(event.id)}, status=status.HTTP_201_CREATED)
    ...
    def get_camera_trap_details(cls, params, exif_dict):
        result = {
            "cameratraprep_camera-name": params.validated_data["camera_name"],
        }
    ...
    def get_time(cls, params, exif_dict):
        try:
            ...
            if "OffsetTimeOriginal" in exif_dict:
                timezone = exif_time_zone(exif_dict["OffsetTimeOriginal"].decode("utf-8"))
