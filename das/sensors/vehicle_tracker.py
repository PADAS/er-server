...
def convert_asset_date(date_str, date_format="%Y-%m-%d %H:%M:%S"):
    ...
    utc_date = obs_fmt.replace(tzinfo=timezone.utc)
...
class TractAdapter:
    ...
    def create_das_object(self, mfg_id, subject_name, tract_record):
        ...
        logger.info("Creeated DAS observation %s", das_obs, extra={"das.obs": das_obs})
        return das_obs
...
class SkylineAdapter:
    ...
    def create_das_object(self, skyline_obs):
        ...
        logger.info("Created DAS observation %s", das_obs, extra={"das.obs": das_obs})
        return das_obs
