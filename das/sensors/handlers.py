...
class GenericSensorHandler():
    ...
    def post(cls, request, sensor_type, provider_key):
        ...
        if Observation.objects.filter(source=src, recorded_at=recorded_at).exists():
            logger.info("Processed duplicate observation %s", subject_subtype, extra={'dup_obs': source_type})
        ...
        if serializer.is_valid():
            serializer.save()
            logger.info("Added new observation %s", observation, extra={'new_obs': source_type})
...
class DasRadioAgentHandler():
    ...
    def handle_observation(cls, data, provider_key):
        ...
        # Short-circuit if we already have this observation.
        if Observation.objects.filter(source=src, recorded_at=recorded_at).exists():
            logger.info("Processed duplicate %s observation", cls.DEFAULT_SUBJECT_SUBTYPE, extra={'dup_obs': cls.SOURCE_TYPE})
            return Response({}, status=status.HTTP_201_CREATED)
        ...
        if serializer.is_valid():
            serializer.save()
            notify_new_tracks(src.id)
            logger.info("Added new observation %s", observation, extra={'new_obs': cls.SOURCE_TYPE})
            return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class GsatHandler():
    ...
    def post(cls, request, provider_key):
        ...
        if serializer.is_valid():
            ...
            notify_new_tracks(src.id)
            logger.info("Processed duplicate %s observation", cls.DEFAULT_SUBJECT_SUBTYPE, extra={'new_obs': cls.SOURCE_TYPE})
