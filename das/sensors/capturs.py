...
class CaptursAdapter:
    ...
    def create_capturs_obs(obs_data):
        ...
        obs = [
            dict(
                ...
                recorded_at=datetime.fromtimestamp(int(data.get("timestamp", data.get("time"))), tz=timezone.utc),
    ...
    def create_das_obs(capturs_obs):
        ...
        """
        das_obs = dict(
            location={"latitude": capturs_obs["lat"], "longitude": capturs_obs["lon"]},
...
class CaptursPushHandler:
    ...
    def post(cls, request, provider_key):
        ...
        if not serializer.is_valid():
            logger.error(f"Invalid observation records: {serializer.errors} , {request.data}")
        ...
        if cls.observations_count:
            return Response(
                data={"message": f"{cls.observations_count} new observations added"}, status=status.HTTP_201_CREATED
            )
    ...
    def ensure_source(cls, capturs_obs):
        ...
        src = Source.objects.ensure_source(
            ...
            manufacturer_id=capturs_obs["device_id"],
            subject={"subject_subtype_id": DAS_SUBJECT_SUBTYPE, "name": capturs_obs["device_id"]},
        )
        return src
    ...
    def create_observations(cls, src, capturs_obs):
        ...
        if Observation.objects.filter(recorded_at=capturs_obs["recorded_at"], source=src).exists():
