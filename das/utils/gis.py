...
def validate_bbox(bbox_as_string):
    ...
    if bbox[0] < -180:
        if bbox[2] > -180:
            left_bbox = Polygon.from_bbox([bbox[0] + 360, bbox[1], 180, bbox[3]])
            right_bbox = Polygon.from_bbox([-180, bbox[1], bbox[2], bbox[3]])
            poly = MultiPolygon(left_bbox, right_bbox)
        else:
            poly = Polygon.from_bbox([bbox[0] + 360, bbox[1], bbox[2] + 360, bbox[3]])
    elif bbox[2] > 180:
        if bbox[0] < 180:
            left_bbox = Polygon.from_bbox([bbox[0], bbox[1], 180, bbox[3]])
            right_bbox = Polygon.from_bbox([-180, bbox[1], bbox[2] - 360, bbox[3]])
            poly = MultiPolygon(left_bbox, right_bbox)
        else:
            poly = Polygon.from_bbox([bbox[0] - 360, bbox[1], bbox[2] - 360, bbox[3]])
    else:
