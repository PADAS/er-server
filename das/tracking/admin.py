...
def _get_plugin_class_search_fields():
    ...
    search_names = [
        f"{c.source_plugin_reverse_relation}__name"
        for n, c in plugin_classes
        if getattr(c, "source_plugin_reverse_relation", None)
    ]
    return search_names
...
class SourcePluginAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Plugin", {"classes": ("wide",), "fields": (("plugin_choice",))}),
        ("Advanced Subject Attributes", {"classes": ("wide", "collapse"), "fields": ("cursor_data",)}),
    )
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "source":
            kwargs["queryset"] = (
                observations.models.Source.objects.all().order_by("manufacturer_id").select_related("provider")
            )
