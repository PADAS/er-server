...
class SourcePluginResult(object):
    ...
    def to_dict(self):
        ...
        return {"count": self.count, "plugin_type": self.plugin_type, "source_id": self.source_id}


class SourcePlugin(TenantModelMixin, TimestampedModel):
    """
    ...
    def execute(self, target=None):
        ...
        if self.should_run:
            ...
            stats.increment(
                "tracking", tags=[f"name:{self.plugin._meta.label_lower}", "state:created"], value=stats_count
            )
...
class PluginTarget(object):
    ...
    def _start(self):
        ...
        def func():
            ...
            try:
                ...
                self.logger.info(
                    "Target received %d messages, created %d items.", accumulator["count"], accumulator["created"]
                )
            except Exception:
    ...
    def __exit__(self, ex_type, exc_value, tb):
        if ex_type not in [None, DasPluginSourceRetryError]:
            self.logger.info("Exiting with Exception. %s %s", ex_type, exc_value, exc_info=(ex_type, exc_value, tb))
...
class DasDefaultTarget(PluginTarget):
    ...
    def _handle_item(self, item):
        ...
        result, created = observations.models.Observation.objects.get_or_create(
            source_id=item.source.id,
            recorded_at=item.recorded_at,
            defaults=dict(location=location, additional=additional, exclusion_flags=item.exclusion_flags),
        )
