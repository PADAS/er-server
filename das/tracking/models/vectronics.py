...
class VectronicsPlugin(TrackingPlugin):
    ...
    def _transform_to_observation(self, source, track_data):
        ...
        if track_data["latitude"] and track_data["longitude"]:
            ...
            side_data = dict((k, track_data.get(k)) for k in track_data.keys() - keys)
            return Obs(
                source=source, latitude=latitude, longitude=longitude, recorded_at=recorded_at, additional=side_data
            )
    ...
    def fetch(self, source, cursor_data, dry_run=False):
        ...
        try:
            observations = self.fetch_observations(
                source.manufacturer_id, source.additional.get("collar_key", ""), after_date
            )
            if dry_run:
                yield observations
            if not dry_run and observations:
                for observation in observations:
                    ...
                    latest_timestamp = max(latest_timestamp, scts) if latest_timestamp else scts
        except Exception as e:
            self.logger.error(e)
