...
# Sample:
sample_record = (33.12635, 3.3208, 300.3, 0.39, 0.44, "2018-03-20", "01:06", "N", "nominal", "1.0NRT", 279.9, 0.6, "N")
...
field_transform = (float, float, float, float, float, str, str, str, str, _trim, float, float, str)
...
class FirmsClient:
    ...
    def calculate_valid_date_indexes(self, stored_headers=None):
        ...
        if stored_dateindex == todays_index:
            return [
                (todays_index, stored_headers),
            ]

        # Continuing from yesterday and starting today.
        if stored_dateindex == yesterdays_index:
            return [(stored_dateindex, stored_headers), (todays_index, None)]
    ...
    def fetch_new_day_records(self, date_index, stored_headers=None):
        ...
        logger.warning(
            "Unexpected response from FIRMS web service..", extra={"url": url, "status_code": data.status_code}
        )
        return [], None
...
class FirmsPlugin(TrackingPlugin):
    ...
    app_key = models.CharField(max_length=1024, blank=True, help_text=app_key_help_text)
    ...
    def execute(self):
        ...
        with DasFireEventTarget() as t:
            for observation in self.fetch():
                t.send(observation)
        self.save()
        logger.info("Finished FIRMS Plugin. region-name=%s", self.firms_region_name)
    ...
    def get_firms_source(self):
        ...
        source, created = Source.objects.get_or_create(
            ...
            manufacturer_id=self.firms_region_name,
    ...
    def get_sourceplugin(self):
        ...
        sourceplugin, created = SourcePlugin.objects.get_or_create(
            defaults={}, source=self.get_firms_source(), plugin_id=self.id, plugin_type=plugin_type
        )
        return sourceplugin
    ...
    def fetch(self):
        ...
        # 'high']
        confidence_alert_levels = self.additional.get("confidence_alert_levels", self.DEFAULT_CONFIDENCE_ALERT_LEVELS)
        ...
        try:
            for observation in self.client.fetch_data(stored_headers=stored_headers):
                if self.pass_filter(observation):
                    ...
                    # observation.
                    if (
                        additional_data.get("confidence", "") in confidence_alert_levels
    ...
    def create_event(self, observation):
        event_details = dict(
            ...
            if k in ("confidence", "frp", "bright_ti4", "bright_ti5", "scan", "track")
        )
        ...
        with transaction.atomic():
            event, created = Event.objects.get_or_create(**event_key, defaults=event_data)
