...
Fix = namedtuple(
    "Fix", ["collar_id", "longitude", "latitude", "recorded_at", "speed", "heading", "temperature", "height"]
)
...
class SpiderTracksClient(object):
    ...
    def fetch_observations(self, start_time):
        ...
        try:
            ...
            self.logger.exception("SpiderTracksClient.fetch_observations failed parsing.")
    ...
    def _get_data(self, start_time):
        ...
        response = requests.request(
            "POST", self.service_api, data=payload, headers=headers, auth=HTTPBasicAuth(self.username, self.password)
        )
        ...
        if response.status_code != 200:
            self.logger.warning(
                "Failed to fetch data from spidertracks with user %s, error %s, message %s",
                self.username,
...
class SpiderTracksPlugin(TrackingPlugin):
    ...
    service_username = models.CharField(max_length=50, help_text="The username for querying the SpiderTracks service.")
    service_password = models.CharField(max_length=50, help_text="The password for querying the SpiderTracks service.")
    service_api = models.CharField(max_length=100, help_text="The API endpoint for the SpiderTracks web-service.")
