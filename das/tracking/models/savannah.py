...
ALERT_EVENT_TYPE_MAP = {
    "immobility alert": {"event_type": "immobility", "title_template": _("{} is immobile")},
    "none": {"event_type": "immobility_all_clear", "title_template": _("{} is moving")},
}
...
class SavannaClient(object):
    ...
    def make_request(self, collar_id, request, record_index=0):
        ...
        payload = dict(
            uid=self.username, pwd=self.password, request=request, collar=collar_id, record_index=record_index
        )
    ...
    def fetch_observations(self, collar_id, last_record_index, last_exception_index):
        ...
        """
        self.logger.info("Fetching from SavannahTracking for collar_id: %s", collar_id)
        while True:
            res = self.make_request(collar_id, "data_download", last_record_index)
            if res.status_code == 200:
                self.logger.info("Fetch OK from SavannahTracking for collar_id: %s", collar_id)
                ...
                msg = "Failed to get data from Savannah Tracking API for collar_id: %s. Result status: %d" % (
                    collar_id,
                    res.status,
                )
    ...
    def fetch_alerts(self, collar_id, last_exception_index):
        ...
        self.logger.info("Getting Savannah collar alarms for collar_id: " "{}".format(collar_id))
        while True:
            alerts_response = self.make_request(collar_id, "exceptions_download", last_exception_index)
            if alerts_response.status_code == 200:
                ...
                for alert in alerts:
                    ...
                    event_type_info = ALERT_EVENT_TYPE_MAP.get(alert_type_lower, None)
    ...
    def parse_line(cls, observation_class, s):
        ...
        """
        dt = (
            (cls.str2date(i) if c == datetime.datetime else c(i))
            for c, i in zip(observation_class.__annotations__.values(), s)
        )
        try:
...
class SavannahPlugin(TrackingPlugin):
    ...
    service_username = models.CharField(
        max_length=50, help_text="The username for querying the Savannah Tracking service."
    )
    service_password = models.CharField(
        max_length=50, help_text="The password for querying the Savannah Tracking service."
    )
    service_api_host = models.CharField(
        max_length=50, help_text="the ip-address or host-name for the Savannah Tracking service."
    )
    ...
    def fetch(self, source, cursor_data=None, dry_run=False):
        ...
        client = SavannaClient(
            username=self.service_username, password=self.service_password, host=self.service_api_host
        )
        ...
        for fix in client.fetch_observations(source.manufacturer_id, last_record_index, last_exception_index):
            ...
            if isinstance(fix, STAlert):
                ...
                # else yield Obs
                obs = Observation.objects.filter(source=source, recorded_at=fix.recorded_at).first()
                if obs:
    ...
    def _transform(self, item, dry_run):
        source, o = item
        side_data = dict(
            (k, o.__getattribute__(k)) for k in ("speed", "heading", "temperature", "height", "hdop", "battery")
        )
        ...
        return Obs(
            source=source, recorded_at=o.recorded_at, latitude=o.latitude, longitude=o.longitude, additional=side_data
        )
