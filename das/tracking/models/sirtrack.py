...
from tracking.models.utils import parse_cookie
...
class SirTrackClient(object):
    ...
    def login(self):
        ...
        login_data = {"id": 2, "method": "loginFrontService.login", "params": [self.username, self.password]}

        try:
            result = requests.post(service_root, json=login_data, timeout=DEFAULT_REQUEST_TIMEOUT)
        except (requests.ConnectionError, requests.ReadTimeout):
            self.logger.exception("Failed to log in to SirTrack API. Service username=%s", self.username)
        else:
            if result.status_code == 200:
                ...
                self.logger.error(
                    "Unable to log in to Sirtrack. result.code: %s, result.data: %s", result.status_code, result.data
                )
    ...
    def get_projects(self, cookies):
        ...
        try:
            ...
            result = requests.get(
                f"{self.service_api}/restlet/projects?_={call_timestamp}",
    ...
    def get_csv_links(self, projects_data):
        ...
        for pd in projects_data:
            kml_url = "{service_api}/restlet/geo/{id}/{name}.kmz".format(service_api=self.service_api, **pd)
    ...
    def get_csv_dataset(self, link):
        ...
        try:
            ...
            self.logger.warning("Failed to read CSV file at %s, ex=%s", link, e)
            raise
        except Exception:
            self.logger.exception("Unexpected error reading CSV file at %s", link)
        finally:
    ...
    def parse_csv_link(self, link):
        ...
        for line in self.get_csv_dataset(link):
            ...
            if not keys:
                ...
                keys = [re.sub("[^a-zA-Z0-9]", "_", k).strip("_").lower() for k in keys]
                continue
...
class SirtrackPlugin(TrackingPlugin):
    ...
    service_username = models.CharField(max_length=50, help_text="The username for querying the SirTrack service.")
    service_password = models.CharField(max_length=50, help_text="The password for querying the SirTrack service.")
    service_api = models.CharField(
        max_length=50, help_text="The API endpoint for SirTrack data.", default="https://data.sirtrack.com"
    )
    ...
    def fetch(self):
        ...
        default_model_name = defaults.get("model_name", self.DEFAULT_MODEL_NAME)

        self.logger = logging.getLogger(self.__class__.__name__)

        client = SirTrackClient(
            username=self.service_username, password=self.service_password, service_api=self.service_api
        )
    ...
    def _transform(self, fix, source):
        ...
        try:
            ...
            self.logger.warning("Failed to transform fix, so ignoring it: %s", fix)
            return None
        ...
        return Obs(source=source, recorded_at=recorded_at, latitude=latitude, longitude=longitude, additional=side_data)
