...
class InreachKMLClient(object):
    ...
    def get_data(self, imei=None, d1=None, d2=None):
        conn = http.client.HTTPSConnection("share.garmin.com", context=ssl._create_unverified_context())
        headers = {
...
class InreachKMLPlugin(TrackingPlugin):
    ...
    service_share_path = models.CharField(max_length=50, help_text="share_path for InReach KML share.")
    service_password = models.CharField(max_length=50, help_text="Password for InReach KML share.")
    service_username = models.CharField(max_length=50, help_text="Username for InReach KML share.")
    ...
    def fetch(self, source, cursor_data=None):
        ...
        client = InreachKMLClient(
            share_path=self.service_share_path, username=self.service_username, password=self.service_password
        )
        ...
        if not dat:
            self.logger.debug("No data found for manufacturer_id %s after %s" % (source.manufacturer_id, latest_ts))
            return
