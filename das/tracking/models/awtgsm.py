...
parser_transforms = {
    "AM": {  # Version 2
        "ntuple": namedtuple(
            "AWTGSM2Fix",
            ["collar_id", "n0", "longitude", "latitude", "recorded_at", "speed", "heading", "temperature", "height"],
        ),
...
class AWTHttpClient(object):
    ...
    def fetch_observations(self, collar_id, start_time, end_time=None):
        ...
        end_time = end_time or datetime.datetime.now(tz=datetime.timezone.utc)
    ...
    def line_parser(cls, collar_id):
        ...
        if not parser_config:
            ...
            raise ValueError(
                "Collar_id %s has a prefix that I don't know about. I expect one of %s "
...
class AWTHttpPlugin(TrackingPlugin):
    ...
    service_api_url = models.URLField(
        help_text="The URL for the AWT service.", default="http://www.yrless.co.za/STE/yrserv/datanew.phtml"
    )
