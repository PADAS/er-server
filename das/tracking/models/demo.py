...
class RandomMover(object):
    ...
    def __init__(
        self, boundaries, initial_position=None, speed_mps=DEFAULT_SPEED_MPS, interval_seconds=DEFAULT_INTERVAL_SECONDS
    ):
        ...
        if initial_position is not None:
            self._current_position = geopy.Point(
                latitude=initial_position["latitude"], longitude=initial_position["longitude"]
            )
        else:
            self._pick_position()
...
class DemoSourcePlugin(TrackingPlugin):
    ...
    range_polygon = models.ForeignKey(mapping.models.PolygonFeature, null=True, on_delete=models.PROTECT)
    ...
    def fetch(self, source, cursor_data=None):
        ...
        try:
            default_starttime = datetime.datetime.now(tz=datetime.timezone.utc) - timedelta(
                seconds=24 * interval_seconds
            )
        ...
        r = RandomMover(
            ...
            initial_position=last_location,
            speed_mps=speed_mps,
        ...
        if observation:
            ...
            self.cursor_data["last_location"] = {"latitude": observation.latitude, "longitude": observation.longitude}
