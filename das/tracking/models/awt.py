...
AWT_DEFAULT_TAG_DATETIME = datetime(year=2010, month=1, day=1, tzinfo=timezone.utc)
...
class AwtClient(object):
    ...
    key_mapping = {"start_time": "T1", "end_time": "T2", "tag_id": "T", "unit": "U"}
    ...
    TOKEN_API = "TOKEN_API"
    APIS = {
        LIVE_API: "/Scripts/php/api/data.php",
    ...
    def __init__(
        ...
        self.redis_client = MultitenantRedisClient(settings.CELERY_BROKER_URL)
    ...
    def check_use_policy(self, api_type, cache_key=None, blocking=True):
        ...
        while True:
            ...
            if cache.get(self.make_major_backoff_key()):
                raise AWTPluginBannedException(f"Banned in check_use_policy for account {self.username}")
    ...
    def set_use_policy_api(self, api_type, major_backoff=False):
        ...
        cache.set(self.make_use_policy_key(api_type), ttl.isoformat(), backoff_seconds)
        if major_backoff:
            cache.set(self.make_major_backoff_key(), ttl.isoformat(), backoff_seconds)
    ...
    def handle_request(self, api_type, url, payload, key=None, expiry_period=None):
        ...
        with redis_utils.lock(
            self.redis_client, self.make_api_lock_key(), self.awt_api_lock_timeout, blocking=True
        ) as l:
            if not l:
                raise AWTPluginFUPBackoffException("Failed to get lock on the awt api")
            ...
            try:
                ...
                self.logger.info(f"AWTPlugin API call {url} account {self.username}")
            ...
            if response.status_code != 200:
                description = "Request status: {0}, Traceback: {1}".format(response.status_code, response.text.strip())
    ...
    def check_and_update_token(self):
        ...
        if awtplugin_data:
            if awtplugin_data["Result"]:
                ...
                raise AWTPluginInvalidSessionTokenException(f"Invalid token result: {awtplugin_data}")
        else:
    ...
    def api_type_for_dates(self, params):
        ...
        if "start_time" in params:
            start_time = datetime.fromtimestamp(params["start_time"], tz=timezone.utc)
    ...
    def fetch_data(self, params=None):
        ...
        if not response:
            ...
            response = self.handle_request(
                api_type, url, payload, key=cache_key, expiry_period=self.fetch_unit_data_expiry
            )
        if response:
            if response["Result"]:
                try:
    ...
    def fetch_units(self):
        ...
        payload = {"ST": self.session_token}
        return self.handle_request(api_type, url, payload, key=key, expiry_period=self.unit_tag_cache_expiry)
    ...
    def fetch_observations(self, params):
        ...
        start_time, end_time = datetime.fromtimestamp(params["start_time"], tz=timezone.utc), datetime.fromtimestamp(
            params["end_time"], tz=timezone.utc
        )
        latest_time = end_time

        while start_time < end_time:
            params["start_time"], params["end_time"] = start_time.timestamp(), end_time.timestamp()
            results = self.fetch_data(params)
            for observation in results:
                if observation["tag_id"] == tag_id:
                    obs_time = datetime.fromtimestamp(observation["timestamp"], tz=timezone.utc)
                    latest_time = obs_time if obs_time > latest_time else latest_time
                    yield observation

            start_time = latest_time if len(results) >= self.max_returned_rows else end_time
...
class AwtPlugin(TrackingPlugin):
    ...
    username = models.CharField(max_length=100, help_text="Username for AWT service.")
    password = models.CharField(max_length=100, help_text="Password for AWT service.")
    host = models.CharField(max_length=100, help_text="API Host for AWT service.")
    subscription_token = models.CharField(max_length=200, help_text="Subscription Token ")
    ...
    def _transform_to_observation(self, source, track_data):
        ...
        if track_data["lat"] and track_data["lon"]:
            ...
            recorded_at = datetime.fromtimestamp(track_data.get("timestamp"), tz=timezone.utc)
            ...
            metadata = track_data
            return Obs(
                source=source, latitude=latitude, longitude=longitude, recorded_at=recorded_at, additional=metadata
            )
    ...
    def _parse_additional_data(self, metadata):
        ...
        fixed_keys = ["api_type", "start_time", "end_time", "tag_id", "unit"]
        if "start_time" in additional_data.keys() and "end_time" in additional_data.keys():
            ...
            if "end_time" in additional_data.keys() or "start_time" in additional_data.keys():
    ...
    def _get_client(self, additional_data={}):
        ...
        use_policy_backoff_threshold = additional_data.get("use_policy_backoff_threshold", None)
        client = AwtClient(
            host=self.host,
    ...
    def fetch(self, source, cursor_data, additional_data={}):
        ...
        try:
            start_date = parse(self.cursor_data["latest_timestamp"]) - COLLAR_REACHBACK_OFFSET
        ...
        for observation in client.fetch_observations(params):
            fix_time = datetime.fromtimestamp(observation.get("timestamp"), tz=timezone.utc)
            obs = self._transform_to_observation(source, observation)
            if obs:
                yield obs

            # keep track of latest timestamp.
            latest_timestamp = max(latest_timestamp, fix_time) if latest_timestamp else fix_time
    ...
    def _sync_unit_info(self):
        ...
        for tag in taglist:
            try:
                name = id = str(tag["id"])
                src, created = ensure_source("tracking-device", id, tag["type"])
                if created:
                    self.logger.info(f"Created source for tag {id}, now creating plugin")
                    ensure_source_plugin(src, self)
                    ensure_subject_source(src, AWT_DEFAULT_TAG_DATETIME, name)
                    break

            except Exception:
                self.logger.exception(f"Error in syncing tag info {tag}")
                raise
...
def ensure_source(source_type, manufacturer_id, model_name="AWT"):
    src, created = Source.objects.get_or_create(
        ...
        defaults={"model_name": model_name, "additional": {"note": "Created automatically during feed sync."}},
    )
