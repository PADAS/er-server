...
class InreachPlugin(TrackingPlugin):
    ...
    service_username = models.CharField(max_length=50, help_text="The username for querying the InReach API service.")
    service_password = models.CharField(max_length=50, help_text="The password for querying the InReach API service.")
    service_api_host = models.CharField(
        max_length=50, help_text="the ip-address or host-name for the InReach API service."
    )
    ...
    class Meta(TrackingPlugin.Meta):
        abstract = False
    ...
    def fetch(self, source, cursor_data=None):
        ...
        self.client = InreachClient(
            host=self.service_api_host, username=self.service_username, password=self.service_password
        )
...
def ensure_source(inreach_device):
    src, created = Source.objects.get_or_create(
        ...
        defaults={
            "model_name": "type:{}, product:{}".format(inreach_device["Type"], inreach_device["Product"]),
...
def ensure_subject_source(source, event_time, subject_name=None):
    ...
    if not subject_source:
        subject_name = subject_name or "inreach-{}".format(source.manufacturer_id)
...
class InreachAccountClient(BasicAuthClient):
    ...
    def user_for_device(self, imei):
        http.client.HTTPSConnection(self.host)
