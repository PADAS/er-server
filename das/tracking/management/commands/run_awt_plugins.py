...
class Command(BaseCommand):
    ...
    SUB_COMMANDS = ("maintenance", "observations", "unitlist", "taglist", "tagsync", "upgrade", "backfill")
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--start-time", help="Start Date(required for getting observation")
        parser.add_argument("--end-time", help="End Date(required for getting observation")

        parser.add_argument(
            "--manufacturer-id", help="Manufacture id(Tag ID), required with " '"observations" sub command'
        )
        parser.add_argument("--unit-id", help="Unit id for AwtPlugin")
        parser.add_argument("--profile", help="AwtPlugin Profile name")

        parser.add_argument(
            "--dry-run", action="store_true", help="stdout data(won't store in DB). Possible " "values [true/false]"
        )
        parser.add_argument(
            "--enable-replay",
            action="store_true",
            help="use awt replay api as needed for 3 month backfill" "values [true/false]",
        )
        parser.add_argument(
            "--enable-history",
            action="store_true",
            help="use awt history api as needed for historical backfill" "values [true/false]",
        )
        parser.add_argument("--output", help="Filename for csv output")
        parser.add_argument("--input", help="Filename for csv input")
    ...
    def fetch_plugins(self, options):
        ...
        if options["profile"]:
            ...
            plugins = self.plugin_class.objects.filter(name=profile_name, status="enabled")
        else:
    ...
    def unitlist(self, options):
        ...
        for plugin in self.fetch_plugins(options):
            awt_client = AwtClient(username=plugin.username, password=plugin.password, host=plugin.host)
    ...
    def validate_start_end_time(self, start, end=None):
        ...
        if start >= datetime.now(tz=timezone.utc):
            raise ValueError("Start time should be less than or equal to " "current time")
        if end:
            if end > datetime.now(tz=timezone.utc):
                raise ValueError("End time should be less than or equal to " "current time")
            if end <= start:
                raise ValueError("End time can't be less than or equal to start" " time")
    ...
    def fetch_observations(self, options, created_callback=None):
        ...
        try:
            ...
            self.logger.error("No source is linked with manufacture_id = " "{0}".format(manufacturer_id))
            raise Exception("No source is linked with manufacture_id = " "{0}".format(manufacturer_id))

        if source:
            for plugin in self.fetch_plugins(options):
                source_plugins = plugin.source_plugins.filter(source=source, status="enabled")
                if source_plugins:
                    for source_plugin in source_plugins:
                        accumulator = None
                        with DasDefaultTarget() as t:
                            created_count = 0
                            for observation in source_plugin.plugin.fetch(source, source_plugin.cursor_data, options):
    ...
    def set_option_times(self, options):
        if not options["start_time"]:
            raise ValueError("start-time is required with end-time. " "Use --start-time [start-time])")

        options["start_time"] = parse(options["start_time"]).replace(tzinfo=timezone.utc)
        options["end_time"] = (
            parse(options["end_time"]).replace(tzinfo=timezone.utc)
            if options["end_time"]
            else datetime.now(tz=timezone.utc)
        )
        self.validate_start_end_time(options["start_time"], options["end_time"])
        return options
    ...
    def observations(self, options):
        ...
        if options["manufacturer_id"]:
            ...
            raise ValueError(
                "Either manufacturer-id or unit-id is required"
    ...
    def backfill(self, options):
        ...
        """
        fieldnames = ("tag_id", "recorded_at", "latitude", "longitude", "source_id")
    ...
    def upgrade(self, options):
        ...
        for plugin in self.fetch_plugins(options):
            ...
            self.logger.info(f"Upgrading existing Skygistics sources to use plugin {plugin.name}")
            response = awt_client.fetch_tags()
            ...
            for source in Source.objects.filter(source_type="tracking-device"):
                mapped_manufacturer_id = convert_skyq_tag_to_awtplugin_tag(source.manufacturer_id)
                ...
                source_provider, created = SourceProvider.objects.get_or_create(provider_key=provider_key)

                try:
                    source_plugin = next(iter(SourcePlugin.objects.filter(source=source)))
