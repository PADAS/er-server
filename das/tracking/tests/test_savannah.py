...
def make_data_download(request_mock, host):
    ...
    request_mock.register_uri(
        "POST", host + "/savannah_data/data_request", json=data, status_code=200, additional_matcher=match_data_download
    )
...
def make_exceptions_download(request_mock, host):
    ...
    request_mock.register_uri(
        "POST",
        host + "/savannah_data/data_request",
        json=data,
        status_code=200,
        additional_matcher=match_exceptions_download,
    )
...
class SavannahPluginTest(TestCase):
    ...
    def setUp(self):
        ...
        self.source_provider = SourceProvider.objects.create(provider_key="savannah", display_name="Savannah")
        self.source = Source.objects.create(
            provider=self.source_provider,
            manufacturer_id="ST2010-3083",
        ...
        plugin_type = ContentType.objects.get(app_label="tracking", model="savannahplugin")
        self.source_plugin = SourcePlugin.objects.create(
            plugin_type=plugin_type, plugin_id=savannah_plugin.id, source=self.source, cursor_data=cursor_data
        )
    ...
    def test_savannah(self, request_mock):
        ...
        self.assertTrue(
            any(observation.__dict__["additional"].get("battery", None) for observation in self.henry.observations())
        )

        # Check alert types immobility, immobility_all_clear in observations
        self.assertTrue(
            any(
                observation.__dict__["additional"].get("device_alert", None) == "immobility"
        ...
        self.assertTrue(
            any(
                observation.__dict__["additional"].get("device_alert", None) == "immobility_all_clear"
                for observation in self.henry.observations()
            )
        )
