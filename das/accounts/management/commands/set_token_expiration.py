...
class Command(TenantCommandMixin, BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "-t", "--token_prefix", type=str, required=True, help="A token with this prefix will be updated."
        )

        parser.add_argument(
            "-x", "--expire_at", type=str, default=None, help="Use this date when updating a token's expiration."
        )
...
def set_token_expiration(token_prefix=None, expire_at=None):
    ...
    try:
        ...
        ac.save()
        logger.info(
            f'Found token with prefix "{token_prefix}" for username {ac.user.username}.'
