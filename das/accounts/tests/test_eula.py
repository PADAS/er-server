...
class EulaModelTestCase(TestCase):
    def setUp(self) -> None:
        ...
        self.user3 = User.objects.create_user(username="user3", password="asdfo9823sfiu23$", email="user3user@user.org")

    def test_only_unique_eula_version_numbers_accepted(self):
        EULA.objects.create(eula_url="http://some.com/eula.pdf", version="EarthRanger_EULA_ver2025-02-12")

        with self.assertRaises(IntegrityError):
            EULA.objects.create(
                eula_url="http://some.com/eula.pdf",
                version="EarthRanger_EULA_ver2025-02-12",
            )

    def test_only_one_active_eula_can_exist_at_any_time(self):
        EULA.objects.create(
            eula_url="http://some.com/eula1.0.pdf", version="EarthRanger_EULA_ver2025-02-12", active=True
        )
        EULA.objects.create(
            eula_url="http://some.com/eula1.1.pdf", version="EarthRanger_EULA_ver2025-03-12", active=True
        )
        latest_eula = EULA.objects.create(
            eula_url="http://some.com/eula1.3.pdf", version="EarthRanger_EULA_ver2025-04-12", active=True
        )
    ...
    def test_get_current_eula_version(self):
        ...
        eula = EULA.objects.create(eula_url="http://some.com/eula1.4.pdf", version="EarthRanger_EULA_ver2025-06-12")
    ...
    def test_get_users_that_agreed_to_current_eula_version(self):
        ...
        self.assertEqual(EULA.objects.get_users_that_have_accepted_the_latest_eula().count(), 1)
...
class EulaViewsTestCase(BaseAPITest):
    ...
    def test_getting_active_eula(self):
        self.tenant_settings.env_settings.accept_eula = True
    ...
    def test_get_eula_returns_200_for_sites_that_dont_accept_eula(self):
        self.tenant_settings.env_settings.accept_eula = False
    ...
    def test_accepted_eula_not_returned_for_sites_not_using_eula(self):
        ...
        response = views.UserView.as_view()(request, id=self.user.id)
        response_data = response.data
        self.assertEqual(response.status_code, 200)
