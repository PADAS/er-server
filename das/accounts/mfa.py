...
def mfa_time_is_fresh(mfa_time: object, max_age_seconds: int) -> bool:
    ...
    epoch value is treated as not fresh. ``MFA_CLOCK_SKEW_SECONDS`` is allowed on top of
    ``max_age_seconds`` to absorb small client/server clock drift.
    """
