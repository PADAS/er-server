...
class Command(TenantCommandMixin, BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        wrapper = SiteMetrics(
            REPORT_TYPE, REPORT_VERSION, reports, site_name, now, start, end, devices, eula, user_session
        )
...
def save_to_bucket(file_contents, start_date, site):
    bucket = S3Boto3Storage(bucket_name=settings.METRICS_BUCKET, default_acl="bucket-owner-full-control")
...
class ExtractSiteMetrics:
    ...
    def get_event_properties(self, event):
        ...
        rep = dict(
            ...
            priority_display=Event.PRIORITY_LABELS_MAP.get(event["priority"], ""),
        )

        rep["event_details"] = self.get_event_detail_properties(event, event_type)
        return rep
    ...
    def get_event_detail_properties(self, event, event_type):
        ...
        current_schema_order = schema_utils.definition_key_order_as_dict(current_schema)
        ...
        for field, order in current_schema_order.items():
            ...
            display = schema_utils.get_display_value_header_for_key(current_schema, field)
...
def sumarize_sources(start, end):
    ...
    for source in queryset:
        ...
        provider["count"] += 1
        provider["model_name"] = provider.get("model_name", None) or source.model_name
        provider["source_type"] = provider.get("source_type", None) or source.source_type
