...
class EulaModelTestCase(TestCase):
    ...
    def test_only_unique_eula_version_numbers_accepted(self):
        EULA.objects.create(url="http://some.com/eula.pdf", version_number=1.0)

        with self.assertRaises(IntegrityError):
            EULA.objects.create(url="http://some.com/eula.pdf", version_number=1.0)

    def test_only_one_active_eula_can_exist_at_any_time(self):
        EULA.objects.create(url="http://some.com/eula1.0.pdf", version_number=1.0,
                                    active=True)
        EULA.objects.create(url="http://some.com/eula1.1.pdf", version_number=1.1,
                            active=True)
        latest_eula = EULA.objects.create(url="http://some.com/eula1.3.pdf", version_number=1.2,
                                    active=True)
    ...
    def test_get_current_eula_version(self):
        EULA.objects.create(url="http://some.com/eula1.0.pdf", version_number=1.0,
                            active=True)
        eula = EULA.objects.create(url="http://some.com/eula1.4.pdf", version_number=1.4)
    ...
    def test_get_users_that_have_acknowledged_eula(self):
        ...
        EULA.objects.accept_eula(user=self.user1)
        self.assertEqual(
            EULA.objects.get_users_that_have_not_accepted_latest_eula().count(), User.objects.count() -1)
...
class EulaViewsTestCase(BaseAPITest):
    ...
    def test_getting_active_eula(self):
        EULA.objects.create(url="http://some.com/eula.pdf", version_number=1.0,
                            active=True)
        eula = EULA.objects.create(url="http://some.com/eulav1.1.pdf",
                                   version_number=1.1)
        ...
        self.assertEqual(eula.url, data.get("url"))
        self.assertEqual(eula.version_number, float(data.get("version_number", 0.0)))
