...
app.conf.task_routes = {
    ...
    "rt_api.tasks.handle_new_source_observation": {
        "queue": "realtime_p3",
    },
...
app.conf.beat_schedule = {
    ...
    "postgresql_partman_run_partition_table_check_for_observations_observationsegment": {
...
def task_success_handler(sender, *args, **kwargs):
    utils.stats.increment("task", tags=[f"name:{sender.name}", "state:success"])


@task_failure.connect
def task_failure_handler(sender, *args, **kwargs):
    utils.stats.increment("task", tags=[f"name:{sender.name}", "state:failure"])
