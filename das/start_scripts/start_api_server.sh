...
function app_has_migrated () {
    local app=$1
    local pattern=$2
    uv run python manage.py showmigrations $app --skip-checks | grep -q "$pattern" && return 0 || return 1
}

function app_in_maintenance_mode () {
    uv run python manage.py maintenancemode status --skip-checks | grep -q "Maintenance mode is enabled" && return 0 || return 1
}

function app_has_pending_migrations () {
    uv run python manage.py showmigrations --skip-checks | grep -q "\[ \]" && return 0 || return 1
}
...
if [[ "${MIGRATIONS_ONLY}" == "True" ]]; then
  ...
  if ! app_has_pending_migrations ; then
    ...
    if app_in_maintenance_mode ; then
      uv run python manage.py maintenancemode disable
  ...
  if app_has_migrated core '\[ \].0008_migrate'; then
      ...
      if ! uv run python manage.py migratewithlock --no-input --settings=das_server.local_settings_oauth_migration; then
      ...
      if ! uv run python manage.py migratewithlock --no-input; then
          echo "Failed to run migratewithlock command, exiting"
          exit 1
      fi
  fi
...
# precedence.
GUNICORN_CMD_ARGS=${GUNICORN_CMD_ARGS:-"--workers 3 --threads 2 --worker-class gthread --max-requests 20000 --max-requests-jitter 500 --timeout 120 --graceful-timeout 40 --keep-alive 10"}
