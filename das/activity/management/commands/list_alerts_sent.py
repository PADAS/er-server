...
class Command(TenantBaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--username", type=str, help="ER username to filter list on")
        parser.add_argument("--output", type=str, help="override where to publish the list")
    ...
    def handle(self, *args, **options):
        ...
        with open(options["output"], "w") as fh:
            ...
            for en in (
                EventNotification.objects.filter(owner__username=username)
                ...
                row = [event_serial, event_title, en.owner, en.method, en.value, en.created_at]
                if en.event:
                    closest = self.find_closest_revision(en.created_at, en.event.revision.all_user())
                    row.extend((closest.action, closest.revision_at.isoformat()) if closest else ("", ""))
