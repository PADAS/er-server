...
class Command(TenantCommandMixin, BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("sub-command", type=str, help="supported commands are {0}".format(Command.SUB_COMMANDS))
        parser.add_argument("--migration-file", type=str, help="input filename for migration plan")
        parser.add_argument("--event-types", nargs="+", help="input list of unused eventtypes to delete")
        parser.add_argument("-o", type=str, help="output filename for dumptypes")
        parser.add_argument("--summary", action="store_true", help="for dumptypes, only output summary data")

        parser.add_argument("--dry-run", action="store_true", help="output result only, do not save to db")

    def migrate_definition(self, event_type):
        schema_raw = event_type.schema
        schema_utils.get_rendered_schema(schema_raw)["properties"]
    ...
    def dumplocalize(self):
        ...
        if not self.output:
            raise NameError("-o output option required")
        self.get_all_event_type_records()
        with open(self.output, mode="w", newline="") as fh:
            ...
            for category in EventCategory.objects.all():
                csv_writer.writerow(dict(ReportCategory=category.value, Display=category.display))

            for event_type in self.get_all_event_type_records():
                event_type_value = event_type["value"]
                csv_writer.writerow(dict(ReportType=event_type_value, Display=event_type["display"]))

                for field_name, field_props in event_type["rendered_schema"]["schema"]["properties"].items():
                    csv_writer.writerow(
                        dict(ReportType=event_type_value, Field=field_name, Display=field_props.get("title"))
                    )
                    for value, display in field_props.get("enumNames", {}).items():
                        csv_writer.writerow(
                            dict(
                                ReportType=event_type_value,
            ...
            for subject_type in SubjectType.objects.all():
                csv_writer.writerow(
                    dict(Model="observations_subjecttype", Value=subject_type.value, Display=subject_type.display)
                )
                for subtype in SubjectSubType.objects.all().filter(subject_type=subject_type):
                    csv_writer.writerow(
                        dict(Model="observations_subjectsubtype", Value=subtype.value, Display=subtype.display)
                    )
                    ...
                    record.update(
                        {
                            ...
                            "tables": self.get_event_type_lookup(event_type.schema, "table"),
                            "queries": self.get_event_type_lookup(event_type.schema, "query"),
                            "enums": self.get_event_type_lookup(event_type.schema, "enum"),
                        }
                    )
    ...
    def get_unused_event_types(self, event_types=None):
        ...
        for event_type in all_event_types:
            count = self.get_event_type_count(event_type)
            if not count:
                logger.info("EventType %s has 0 records associated with it", event_type.value)
    ...
    def migratetypes(self):
        ...
        if self.dry_run:
            raise Exception("Just-in-case exception to prevent atomic operation from completing")
    ...
    def perform_migration_on_records(self, records):
        ...
        for record in records:
            try:
                ...
                logger.exception("Exception while migrating types or choice table fields")
                raise
        ...
        for record in records:
            try:
                ...
                self.update_fields_with_event_type(record, event_type, migrated_tables)
            except Exception:
    ...
    def render_schema(self, schema, include_enums=False):
        ...
        if not include_enums:
            return schema_utils.render_schema_template(schema, schema_utils.get_empty_params(schema))
    ...
    def get_event_type_fields(self, schema):
        ...
        fields = schema_utils.get_all_field_names(schema)
        return fields
        ...
        for field in schema_utils.get_replacement_fields_in_schema(schema):
            if field[schema_utils.LOOKUP_ATTR] == lookup:
                if field[schema_utils.LOOKUP_ATTR] == "table":
                    lookups.append({"table_name": field[schema_utils.FIELD_ATTR]})
                else:
        ...
        try:
            ...
            with connection.cursor() as conn:
                ...
                if "schema" in record and record["schema"]:
                    ...
                    conn.execute(
                        "UPDATE activity_eventtype SET schema = %s WHERE id = %s", [record["schema"], new_event_type.id]
                    )
        ...
        if old_event_type_value:
            ...
            for event in Event.objects.filter(event_type_id=old_event_type.id):
                with connection.cursor() as cursor:
                    cursor.execute(
                        "UPDATE activity_event SET event_type_id = %s WHERE id = %s", [new_event_type.id, event.id]
                    )

            with connection.cursor() as conn:
                conn.execute("DELETE FROM activity_eventtype WHERE id = %s", [old_event_type.id])

    def create_new_event_type(self, event_type_data):
        category = EventCategory.objects.get(value=event_type_data["category_value"])
    ...
    def should_lookup_value_for_field(
        self, record, previous_property_name, current_property_name, former_tables, old_data
    ):
    ...
    def lookup_choice_value(self, table_row_id):
        ...
        raise ChoiceNotFoundException("did not find choice by id {table_row_id}")
    ...
    def is_choice_property(self, property_id, rendered_schema):
        ...
        try:
            ...
            logger.warning(f"Property {property_id} not found in event type {rendered_schema['schema']['title']}")
            return False
    ...
    def _modify_event_details(self, data, migration_plan, event_type, former_tables):
            ...
            for property_id, details in data.items():
                ...
                if isinstance(details, str):
                    if self.is_uuid(details):
                        try:
                            data[property_id] = self.lookup_choice_value(UUID(details))
                            dirty = True
                        except ChoiceNotFoundException:
                            # ignore that this might be a query lookup
                            logger.warning(f"Choice not found for property {property_id} - {details}")
                else:
                    if isinstance(details, dict):
                        details = [details]
                    if not isinstance(details, list):
                        raise ChoiceException(f"Unknown details type: {details}")
                    for item in details:
                        if self.is_uuid(item["value"]):
                            try:
                                item["value"] = self.lookup_choice_value(UUID(item["value"]))
                                dirty = True
                            except ChoiceNotFoundException:
        ...
        if self.should_update_fields_with_event_type(migration_plan["fields"]):
            for field in migration_plan["fields"]:
                if self.PREVIOUS_PROPERTY_FIELD in field:
                    previous_property_name = field.get(self.PREVIOUS_PROPERTY_FIELD, self.COMMAND_IGNORE)
                    property_name = field.get(self.CURRENT_PROPERTY_NAME)
                    property_value = field.get(self.CURRENT_PROPERTY_VALUE) or data[previous_property_name]
                    ...
                    try:
                        ...
                        if previous_property_name in data:
                            if self.should_lookup_value_for_field(
                                migration_plan, previous_property_name, property_name, former_tables, data
                            ):
                                try:
                                    choice_object = choices.Choice.objects.get(id=data[previous_property_name])
                                    data[property_name] = str(choice_object.value)
                                except TypeError:
    ...
    def update_fields_with_event_type(self, migration_plan, event_type, former_tables):
        for event in Event.objects.filter(event_type_id=event_type.id):
            for event_details_revision in event.revision.all():
                ...
                details, dirty = self._modify_event_details(details, migration_plan, event_type, former_tables)
                if dirty:
                    data["data"]["event_details"] = details
                    with connection.cursor() as cursor:
                        cursor.execute(
                            "UPDATE activity_eventdetailsrevision SET data = %s WHERE id = %s",
            ...
            for event_details in event.event_details.all():
                ...
                data, dirty = self._modify_event_details(data, migration_plan, event_type, former_tables)
                if dirty:
                    with connection.cursor() as cursor:
                        cursor.execute(
                            "UPDATE activity_eventdetails SET data = %s WHERE id = %s",
                            [json.dumps({"event_details": data}), event_details.id],
                        )

    def migrate_choices_table(self, table_name, model, field):
        table_ct = ContentType.objects.get(app_label="choices", model=table_name)
        ...
        for row in table.objects.all():
            ...
            if choice_row:
                ...
                existing_choice = choices.Choice.objects.filter(
                    model=model, field=field, value=self.make_value(row.name)
                ).first()

                if existing_choice:
                    if existing_choice.display != row.name:
                        raise ChoiceException(
                            f"Found matching Choice row by {model}:{field}:{self.make_value(row.name)},"
                    ...
                    if existing_choice.id != row.id:
                        add_to_choice_mapping(row.id, row.name, existing_choice.id)
                        logger.info(
                    ...
                    logger.debug("New choice %s, migrated from %s table to choices", new_choice.value, table_name)
