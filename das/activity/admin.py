...
class EventTypeAdmin(BaseModelAdminMixin):
    ...
    def _default_priority_display(self, obj):
        url = models.Event.marker_icon(obj.icon_id, obj.default_priority, models.Event.SC_NEW)
        priority_name = models.Event.PRIORITY_LABELS_MAP.get(obj.default_priority, models.Event.PRI_NONE)
    ...
    def get_event_type_ref(self, object_id):
        try:
            ...
            if event_type is not None:
                return {
                    "href": reverse(
                        f"admin:{event_type._meta.app_label}_{event_type._meta.model_name}_change",
                        args=(event_type.id,),
                    ),
...
class RefreshRecreateEventDetailViewAdmin(BaseModelAdminMixin):
    ...
    formfield_overrides = {JSONField: {"widget": PrettyReadOnlyJSONWidget}}
    ...
    def refresh_view(self, request):
        ...
        name = "refresh"
        return self.manage_task_status(request=request, task=task, task_mode=task_mode, qs_method=qs_method, name=name)
...
class PatrolPermissionMixin:
    ...
    def has_view_permission(self, request, obj=None):
        ...
        return request.user.has_perm(f"{opts.app_label}.{codename_view}") or request.user.has_perm(
...
class PatrolAdmin(PatrolPermissionMixin, OSMGeoExtendedAdmin):
    ...
    def get_queryset(self, request):
        ...
        overdue = (
            Q(patrol_segment__scheduled_start=F("patrol_segment__scheduled_start"), state=models.PC_OPEN)
            & Q(patrol_segment__time_range__startswith__isnull=True)
            & Q(patrol_segment__scheduled_start__lt=set_time)
        )

        readyto = Q(state=models.PC_OPEN) & Q(
            patrol_segment__time_range__startswith__range=(present_time, end_day)
        ) | Q(patrol_segment__scheduled_start__range=(set_time, end_day))

        scheduled = Q(state=models.PC_OPEN) & (
            Q(patrol_segment__time_range__startswith__gt=end_day) | Q(patrol_segment__scheduled_start__gt=end_day)
        )

        queryset = queryset.annotate(
            patrol_type=Subquery(patrol_sgment.values("patrol_type__display")[:1]),
            tracked_subject=Subquery(
                patrol_sgment.annotate(leader_name=Subquery(subject.filter(Q(id__in=subjects)).values("name"))).values(
                    "leader_name"
                )[:1]
            ),
            tracked_user=Subquery(
                patrol_sgment.annotate(leader_name=Subquery(user.filter(Q(id__in=users)).values("username"))).values(
                    "leader_name"
                )[:1]
            ),
            ...
            status=Case(
                When(overdue, then=Value(PatrolState.overdue.value)),
    ...
    def changeform_view(self, request, object_id=None, form_url="", extra_context=None):
        try:
            ...
            self.message_user(
                request, _("Actual start date must be earlier or equal to Actual end date"), level=messages.ERROR
            )
            return HttpResponseRedirect(request.get_full_path())

    @staticmethod
    def search_tracked_subject(search_term):
        q_object = Q(models.Subject.objects.filter(name__icontains=search_term)) | Q(
