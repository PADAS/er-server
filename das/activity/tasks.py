...
def send_alert_to_notificationmethod(
    ...
    if features.tms.is_on():
        ...
        default_from_email = tenant_data.get().get("envSettings", {}).get("defaultFromEmail")
        send_event_alert(
...
def refresh_event_details_view_task(activity):
    ...
    last_five_ids = [rec.id for rec in RefreshRecreateEventDetailView.objects.order_by("-started_at")[:5]]
    RefreshRecreateEventDetailView.objects.filter(started_at__lte=minimum_date).exclude(id__in=last_five_ids).delete()

    if check_db_view_exists():
        (
            refresh_event_details_view.s(activity=activity) | update_status_of_event_details_view_refresh.s()
        ).apply_async()


@celery.app.task(bind=True)
def update_status_of_event_details_view_refresh(self, activity_and_status):
    logger.info("updating status of event details view refresh: %s", activity_and_status)
    activity, status, error_details = activity_and_status

    RefreshRecreateEventDetailView.objects.create(
