...
* ``reconstruct_from_json`` — V1 not reconstructable; hand the V2
                              ...
                              (pinned at ==0.1.5.2 in ``pyproject.toml``),
...
# NOTE: DASTenant is imported LIVE (not via apps.get_model) on purpose.
# It is a django-multitenant TenantModel whose tenant config does NOT
# survive migration-state rendering, so the historical "__fake__" model
# raises AttributeError when its rows are constructed. EventType/
# EventTypeRevision come from apps.get_model (EventType survives; the
# revision rows are read via values_list to avoid construction — see
# fetch_in_migration). Do NOT "fix" this to apps.get_model("core","DASTenant").
        ...
        for event_type in v2_event_types:
            try:
                ...
                if outcome.did_apply:
                    ...
                    # bug that the repair tool won't auto-fix. Log at warning so
                    # operators can build a manual-review queue from structured
                    # logs (the metadata carries the diagnostic change summary).
                    logger.warning(
                    ...
                    # log at debug to avoid flooding deploys.
                    logger.debug(
