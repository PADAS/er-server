...
class TestEventView(BaseTestToolMixin, BaseAPITest):
    ...
    def setUp(self):
        ...
        self.guest_user = User.objects.create_user(
            "guest_user", "das_guest_user@vulcan.com", "guest_user", **self.user_const
        )
        self.radio_room_user = User.objects.create_user(
            "radio_room_user", "das_radio_room@vulcan.com", "radio_room_user", **self.user_const
        )
        self.power_user = User.objects.create_user(
            "power_user", "das_power_user@vulcan.com", "power_user", **self.user_const
        )
        self.all_perms_user = User.objects.create_user(
            "all_perms_user", "das_all_perms@vulcan.com", "all_perms_user", **self.user_const
        )

        self.eventsource_user_no1 = User.objects.create_user(
            "eventsource_user_no1", "eventsource_user_no1@tempuri.org", "eventsource_user_no1", **self.user_const
        )

        self.eventsource_user_no2 = User.objects.create_user(
            "eventsource_user_no2", "eventsource_user_no2@tempuri.org", "eventsource_user_no2", **self.user_const
        )
    ...
    def test_add_note(self):
        ...
        request = self.factory.post(self.api_base + "/event/{0}/notes".format(self.sample_event.id), note_data)
        self.force_authenticate(request, self.all_perms_user)

        response = views.EventNotesView.as_view()(request, id=str(self.sample_event.id))
        self.assertEqual(response.status_code, 201)
        response_data = response.data
    ...
    def test_update_note_using_patch(self):
        ...
        request = self.factory.patch(self.api_base + "/event/{0}".format(self.sample_event.id), event_data)
        self.force_authenticate(request, self.all_perms_user)

        response = views.EventView.as_view()(request, id=str(self.sample_event.id))
        self.assertEqual(response.status_code, 200)
        response_notes = response.data["notes"]
        self.assertEqual([note for note in response_notes if note["id"] == str(note_id)][0]["text"], note_text)
    ...
    def test_add_note_using_patch(self):
        ...
        self.assertEqual(response.status_code, 200)
        response_notes = response.data["notes"]
        self.assertTrue(len(list(filter(lambda note: note["text"] == note_data["text"], response_notes))) == 1)
    ...
    def test_update_message_succeed(self):
        ...
        request = self.factory.patch(self.api_base + "/event/{0}/".format(str(event.id)), update_data)
        self.force_authenticate(request, self.all_perms_user)
    ...
    def test_event_feed_filter_contained_events(self, is_banned):
        ...
        EventRelationship.objects.add_relationship(from_event=incident, to_event=contained_event, type="contains")

        request = self.factory.get(self.api_base + "/events?exclude_contained=true")
        ...
        response_data = response.data
        self.assertEqual(response.status_code, 200)
        self.assertFalse(["failed" for r in response_data["results"] if len(r["is_contained_in"])])

    def test_event_type_category(self):
        request = self.factory.get(self.api_base + "/events/eventtypes?category=monitoring&event_category=security")
        self.force_authenticate(request, self.all_perms_user)
    ...
    def test_add_reported_by(self):
        ...
        self.assertEqual(response.status_code, 200)
        response_data = response.data
        self.assertEqual(response_data["reported_by"]["id"], update_data["reported_by"]["id"])
    ...
    def test_update_event_state_active(self):
        ...
        request = self.factory.patch(self.api_base + "/event/{0}/state".format(str(event.id)), update_data)
        self.force_authenticate(request, self.all_perms_user)

        response = views.EventStateView.as_view()(request, id=str(event.id))
        self.assertEqual(response.status_code, 200)
        response_data = response.data
    ...
    def test_collection_event_contains_with_different_user_permissions(self):
        ...
        event_collection = Event.objects.create(title="incident_collection_event", event_type=collection_et)
        event_a = Event.objects.create(title="Event_A", event_type=logistics_et)
        event_b = Event.objects.create(title="Event_B", event_type=monitoring_et)
        ...
        request = self.factory.get(self.api_base + "/event/" + str(event_collection.id))
        self.force_authenticate(request, self.all_perms_user)
        ...
        contained_event_titles = [k.get("related_event").get("title") for k in contained_events]
        ...
        new_request = self.factory.get(self.api_base + "/event/" + str(event_collection.id))
        self.force_authenticate(new_request, self.guest_user)
        new_response = views.EventView.as_view()(new_request, id=str(event_collection.id))
    ...
    def test_edit_event_details(self):
        ...
        request = self.factory.patch(self.api_base + "/event/{0}".format(str(event_id)), update_data)
        self.force_authenticate(request, self.all_perms_user)
    ...
    def _export_template_response(self, request):
        return views.EventsExportView.as_view(
            content_type="text/csv", template_engine="jinja2", template_name="event_export_template.html"
        )(request)
    ...
    def test_collection_report_id_exported_as_parent_event_serial_number(
        ...
        request = self.factory.post(self.api_base + "/events/", collection_event_data)
        self.force_authenticate(request, self.all_perms_user)
        ...
        response_data = {k: response_data[k] for k in collection_event_data.keys()}
    ...
    def test_export_reports_with_create_date_filter(self):
        url = """/activity/events/export"""
        q_params = json.dumps(
            {"create_date": {"lower": self.start_of_today.isoformat(), "upper": self.end_of_today.isoformat()}}
        )
    ...
    def test_filter_events_with_update_date(self, is_banned):
        ...
        url = """/activity/events?"""
        q_params = json.dumps(
            {"update_date": {"lower": self.start_of_today.isoformat(), "upper": self.end_of_today.isoformat()}}
        )
        ...
        q_params = json.dumps({"update_date": {"lower": self.start_of_today.isoformat()}})
    ...
    def test_export_filter_on_incident_associated_reports(self):
        ...
        self.assertTrue(all(x in report_names for x in [incident_data["title"], self.event_data["title"]]))
    ...
    def test_export_includes_all_event_detail_fields_no_title(self):
        ...
        details.data = {"event_details": {"eLocust-key": "e locust id key", "repObserver": "an observer"}}
        details.save()
    ...
    def test_export_csv_with_line_feed(self):
        currentactivity = "\n".join((lorem_ipsum.paragraph(), lorem_ipsum.paragraph()))
    ...
    def test_add_event_category(self):
        ...
        permissionset_list = PermissionSet.objects.filter(name=expected_permissionset_name)
        ...
        for operation in ACTIONS:
            ...
            self.assertTrue(permission_list[0] in permissionset_list[0].permissions.all())
    ...
    def do_all_operations_on_all_event_types(self, user):
        ...
        result.update(self.do_all_event_operations(user, ET_MONITORING, "monitoring"))
    ...
    def test_eventprovider_permissions(self, get_tenant_settings):
        ...
        response = views.EventProvidersView.as_view()(
            request,
        )
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.data["count"], 0)

        EventProvider.objects.filter(id=eventprovider.id).update(is_active=True)
        request = self.factory.get(f"{self.api_base}/activity/eventproviders")
        self.force_authenticate(request, self.eventsource_user_no1)
        ...
        result_eventprovider = {k: response.data["results"][0][k] for k in eventprovider_data.keys()}
        self.assertEqual(response.data["results"][0]["id"], str(eventprovider.id))
    ...
    def test_get_existing_eventsource(self):
        ...
        response = views.EventSourceView.as_view()(
            request, eventprovider_id=str(eventprovider.id), external_event_type=external_event_type
        )
    ...
    def test_cannot_see_another_users_eventprovider(self, get_tenant_settings):
        ...
        self.assertEqual(response.data["results"][0]["id"], str(eventprovider_no1.id))
        ...
        self.assertEqual(response.data["results"][0]["id"], str(eventprovider_no2.id))
    ...
    def test_add_identical_external_event_id_with_different_event_providers(self):
        ...
        request = self.factory.post(
            f"{self.api_base}/activity/eventprovider/{str(eventprovider_no1.id)}/eventsources", eventsource_data
        )
        self.force_authenticate(request, self.eventsource_user_no1)
        response = views.EventSourcesView.as_view()(request, eventprovider_id=str(eventprovider_no1.id))
        self.assertEqual(response.status_code, 201)
        esid_no1 = response.data["id"]

        # Create event source for provider No. 2
        request = self.factory.post(
            f"{self.api_base}/activity/eventprovider/{str(eventprovider_no2.id)}/eventsources", eventsource_data
        )
        self.force_authenticate(request, self.eventsource_user_no1)
        response = views.EventSourcesView.as_view()(request, eventprovider_id=str(eventprovider_no2.id))
        self.assertEqual(response.status_code, 201)
        ...
        event_type_no2 = EventType.objects.create(
            value="eventsource_no2_event_type", display="eventsource_no2_event_type", category=event_category
        )
        EventSource.objects.filter(eventprovider_id=str(eventprovider_no2.id), id=esid_no2).update(
    ...
    def test_array_property_name_same_as_child_enum_name(self):
        et_schema = schema_examples.CONFISCATION_SCHEMA
        event_type = self.sample_event.event_type
        event_type.schema = et_schema
        event_type.save()
        ...
        choice = Choice.objects.create(
            ...
            field="confiscationrep_itemsconfiscated",
            value="badger",
            display="Badger",
        )
        ...
        event_data["event_details"] = {
            "confiscationrep_details": lorem_ipsum.paragraph(),
            "confiscationrep_itemsconfiscated": [{"ItemConfiscated": "badger", "ItemNumber": 5}],
        }
        request = self.factory.post(self.api_base + "/events/", event_data)
        self.force_authenticate(request, self.all_perms_user)
        response = views.EventsView.as_view()(request)
    ...
    def test_property_checkboxes(self):
        ...
        event_data["event_type"] = ET_OTHER
        event_data["event_details"] = {
            "carcassrep_species": ["elephant", "eland"],  # checkboxes
            "sectionArea": "unknown",
            "conservancy": "unknown",
            # multi-select
        ...
        event_detail = EventDetails.objects.create(
            ...
            data={
                "event_details": {
                    ...
                    "sectionArea": [{"name": "Area1", "value": "area1"}, {"name": "Area2", "value": "area2"}],
                    "arrestrep_reasonforarrest": ["snare", "logging"],
                }
            },
        )
    ...
    def test_search_filter_with_one_event_id_returns_none(self, is_banned):
        ...
        query = {"filter": json.dumps({"text": title_search_text}), "event_ids": [event_id]}
    ...
    def test_search_filter_with_two_event_id_returns_one(self, is_banned):
        ...
        query = {"filter": json.dumps({"text": title_search_text}), "event_ids": event_ids}
    ...
    def test_search_event_by_eventtype_and_active(self, is_banned):
        ...
        url = self.api_base + "/events/?" + urlencode(qparams)
        request = self.factory.get(url)
        self.force_authenticate(request, self.all_perms_user)
        response = views.EventsView.as_view()(request)
        self.assertTrue(response.data)
    ...
    def test_eventnote_generate_tsvector_doc(self, is_banned):
        ...
        event_note = dict(id="747b4d5a-79a3-11ea-bc55-0242ac130003", text=lorem_ipsum.paragraph())
    ...
    def test_event_note_text_search(self, is_banned):
        ...
        uuid = response.data["id"]
        url = reverse("event-view-notes", args=(uuid,))
        event_note = dict(id="747b4d5a-79a3-11ea-bc55-0242ac130003", text="This is an example of a note.")
    ...
    def test_checkbox_field_values_included_in_export(self):
        et_schema = json.dumps(
            {
                "schema": {"properties": {"rhinosightingrep_unknownpicklist": {"key": "rhinosightingrep_unknown"}}},
                "definition": [
                    {
                        "type": "fieldset",
                        "htmlClass": "col-lg-6",
        ...
        EventDetails.objects.create(
            data={"event_details": {"rhinosightingrep_unknownpicklist": ["unknown_rhino_1"]}}, event=self.sample_event
        )
    ...
    def test_export_with_0_event_details_data(self):
        et_schema = json.dumps(
            {
                "schema": {
                    ...
                    "properties": {
                        ...
                        "test_four_number": {"type": "number", "title": "Test 4 Number"},
                    },
                },
        ...
        EventDetails.objects.create(
            data={"event_details": {"test_four_number": 0, "test_three_number": 0}}, event=self.sample_event
        )
    ...
    def test_export_on_checkbox_with_query_titlemaps(self):
        ...
        EventDetails.objects.create(data={"event_details": {"kitten": [str(subject.id)]}}, event=self.sample_event)
    ...
    def test_schema_with_inactive_choices(self):
        ...
        choice = Choice.objects.create(
            model="activity.event", field="wildlifesighting_species", value="baboon", display="Baboon", is_active=False
        )
        choice.save()
        choices.append(choice)
        ...
        assert response.status_code == 200
        species_display = [
            display_prop for display_prop in response.data["definition"] if display_prop.get("key", "") == "species"
        ][0]
        assert "inactive_titleMap" in species_display

        species_display = [
            display_prop
            for display_prop in response.data["definition"][0]["items"]
            if isinstance(display_prop, dict) and display_prop.get("key", "") == "animal_species"
        ][0]
        assert "inactive_titleMap" in species_display
    ...
    def test_schema_with_same_inactive_choices(self):
        ...
        request = self.factory.get(url)
        self.force_authenticate(request, self.all_perms_user)
        new_response = views.EventTypeSchemaView.as_view()(request, eventtype=event_type.value)
    ...
    def test_flat_definition(self):
        ...
        assert (
            len(
                [
                    display_prop
                    for display_prop in response.data["definition"]
                    if isinstance(display_prop, dict) and display_prop.get("type") == "fieldset"
                ]
            )
            == 0
        )
        assert (
            len(
                [
                    display_prop
                    for display_prop in response.data["definition"]
                    if isinstance(display_prop, str) and display_prop in ("reportlocationarea", "reportreportername")
                ]
            )
            == 3
        )
    ...
    def test_schema_with_string_arrays(self):
        ...
        base_schema = {
            "schema": {
                "properties": {"MusthBull": {"title": "Musth Bull Present", "type": "string", "enum": ["yes", "no"]}},
                "definition": ["MusthBull"],
            }
        }
        event_type = self.sample_event.event_type
        ...
        assert (
            response.data["schema"]["properties"]["MusthBull"]["title"]
            == base_schema["schema"]["properties"]["MusthBull"]["title"]
        )
...
class TestParsing(TestCase):
    def test_dates(self):
        ...
        val = dict(lower=lower.isoformat(), upper=upper.isoformat())
...
class TestEventFilterQueryset:
    ...
    def test_by_location_filter(
        ...
        assert (
            Event.objects.by_location(request.GET.get("location", ""), request.user, categories_to_search).exists()
            == known_location["result"]
        )
...
class TestEventFilterQuerysetByBbox:
    ...
    def _events_with_geometries(self, five_events, five_event_geometries):
        ...
        for event_geometry, polygon in [(park, FLOWER_MILL_PARK), (lake, TEXCOCO_LAKE)]:
    ...
    def test_bbox_includes_both_geometries_and_locations(self, _events_with_geometries):
    ...
    def test_bbox_excludes_geometries_out_of_boundaries(self, _events_with_geometries):
    ...
    def test_bbox_includes_partial_overlapping_event(self, _events_with_geometries):
    ...
    def test_bbox_excludes_all_geometries_and_locations(self, _events_with_geometries):
