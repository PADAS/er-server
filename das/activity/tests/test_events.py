...
class TestEventView(BaseAPITest):
    ...
    def do_all_event_operations(self, user, event_type, event_type_name):
        ...
        response = views.EventsView.as_view()(request)
        results['{0}_create'.format(event_type_name)] = response.status_code

        # Because we don't know if the previous evx ent creation failed, create
        ...
        request = self.factory.get(self.api_base + '/activity/event/' + str(event.id))
        self.force_authenticate(request, user)
        response = views.EventView.as_view()(request)
        results['{0}_read'.format(event_type_name)] = response.status_code

        # Attempt to modify the event we just created
        event_data['message'] = lorem_ipsum.paragraph(),
        request = self.factory.patch(self.api_base + '/activity/event/{0}'.format(event.id), event_data)
        self.force_authenticate(request, user)
        response = views.EventView.as_view()(request)
        results['{0}_update'.format(event_type_name)] = response.status_code

        # Attempt to delete the event we just created
        request = self.factory.delete(self.api_base + '/activity/event/{0}'.format(event.id))
        self.force_authenticate(request, user)
        response = views.EventView.as_view()(request)
        results['{0}_delete'.format(event_type_name)] = response.status_code
