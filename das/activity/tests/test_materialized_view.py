...
class TestMaterializedView(BaseAPITest):
    ...
    def test_when_admin_refresh_view(self, mocked_tenant_client, get_tenant_settings):
        ...
        self.assertEqual(messages._queued_messages[0].message, "Successfully refresh 'event_detail_view'")
    ...
    def test_when_admin_recreate_view(self, mocked_tenant_client, get_tenant_settings):
        ...
        self.assertEqual(messages._queued_messages[0].message, "Successfully recreate 'event_detail_view'")
        self.assertEqual(response.status_code, 302)
    ...
    def test_enum_values_returned_instead_of_names(self):
        ...
        EventDetailsItem = namedtuple(
            "EventDetailsItem", "subjects_name behavior_choice behavior sample_attr expected_report_tuple"
        )

        event1 = EventDetailsItem(
        ...
        EventDetails.objects.create(
            ...
            event=Event.objects.create(
                title="test event with new format",
        ...
        EventDetails.objects.create(
            ...
            event=Event.objects.create(
                title="test event old format", event_type=self.event_type, created_by_user=self.app_user, state="new"
            ),
        )
