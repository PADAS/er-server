...
class TestPatrol(BaseAPITest):
    def setUp(self):
        ...
        self.app_user = User.objects.create_user(
            "app-user2", "app-user2@test.com", "app-user2", is_superuser=True, is_staff=True, **user_const
        )
            ...
            patrol_segments=[
                {
                    ...
                    "start_location": {"longitude": -122.3607072, "latitude": 47.681731199999994},
                    "end_location": {"longitude": -124.3607072, "latitude": 49.681731199999994},
                }
            ],
        )
    ...
    def test_add_note(self):
        ...
        request = self.factory.post(self.api_base + "/patrols/{0}/notes".format(self.sample_patrol_id), note_data)
        self.force_authenticate(request, self.app_user)

        response = views.PatrolNotesView.as_view()(request, id=str(self.sample_patrol_id))
        self.assertEqual(response.status_code, 201)
        response_data = response.data
        response_data = {k: response_data[k] for k in note_data.keys()}
        self.assertDictEqual(response_data, note_data)

        request = self.factory.get(self.api_base + f'/patrols/{self.sample_patrol_id}/notes/{response.data["id"]}')
        self.force_authenticate(request, self.app_user)

        response = views.PatrolNotesView.as_view()(
    ...
    def test_create_patrol_and_upload_document(self):
        ...
        with open(filename, "rb") as f:
            path = "/".join((self.api_base, "activity", "patrols", my_patrol_id, "files"))
        ...
        path = "/".join((self.api_base, "activity", "patrols", my_patrol_id, "files", file_id, file_name))
        request = self.factory.get(path)
        self.force_authenticate(request, self.app_user)
            ...
            patrol_segments=[
                {
                    ...
                    "time_range": {"start_time": "2020-09-24T07:08:16.711000+03:00"},
                    "start_location": {"longitude": -122.3607072, "latitude": 47.681731199999994},
                }
            ],
        )
            ...
            patrol_segments=[
                {
                    ...
                    "time_range": {"end_time": self.now.isoformat()},
                    "end_location": {"longitude": 37.41343018527925, "latitude": 0.17796830457972135},
                }
            ],
        )
            ...
            patrol_segments=[
                {
                    ...
                    "time_range": {"start_time": "2020-09-24T07:08:16.711000+03:00", "end_time": None},
                    "start_location": None,
                    "end_location": {"longitude": -122.3607072, "latitude": 47.681731199999994},
    ...
    def test_update_patrol_with_new_patrolsegment(self):
        ...
        patrolsg = dict(patrol_type="dog_patrol", patrol=self.default_test_patrol.id)
    ...
    def test_patrol_filter_with_patrols_overlap_daterange_param(self):
        ...
        updated_time_range = DateTimeTZRange(lower=self.start_of_today, upper=self.end_of_today)

        PatrolSegment.objects.filter(id=segment_id).update(time_range=updated_time_range)
        new_filter = {"filter": json.dumps({"date_range": date_range})}
    ...
    def test_patrol_filter_by_state(self):
        ...
        url = reverse("patrols") + f"?status=scheduled&status=active&status=done&status=cancelled"
    ...
    def test_patrol_filter_with_past_end_time_but_patrol_not_completed(self):
        ...
        patrol_data = dict(
            title="Test Patrol",
            patrol_segments=[
                {"time_range": {"start_time": start.isoformat()}, "scheduled_end": scheduled_end.isoformat()}
            ],
        )
        self._create_patrol(patrol_data)
        response = self._filter_patrol(self.sample_patrol_filter)
        ...
        patrol_update_data = dict(
            state="done",
            patrol_segments=[
                {"id": result.get("patrol_segments")[0]["id"], "time_range": {"end_time": scheduled_end.isoformat()}}
            ],
        )
        request = self.factory.patch(url, data=patrol_update_data)
        self.force_authenticate(request, self.app_user)
    ...
    def test_overdue_readytostart(self):
        ...
        readytostart = dict(title="readytostart", patrol_segments=[{"scheduled_start": ahead.isoformat()}])
    ...
    def test_sort_overdue_readytostart_active(self):
        ...
        active_patrol = dict(title="active", patrol_segments=[{"time_range": {"start_time": now.isoformat()}}])
    ...
    def test_sort_by_state(self):
        ...
        future_patrol = dict(title="future patrol", patrol_segments=[{"scheduled_start": future_scheduled.isoformat()}])

        cancel_readytostart = dict(
            title="cancelled readytostart", state="cancelled", patrol_segments=[{"scheduled_start": ahead.isoformat()}]
        )
        cancel_readytostart2 = dict(
            title="cancelled readytostart_control",
            state="cancelled",
        ...
        active_patrol0 = dict(
            title="active B",
            patrol_segments=[{"time_range": {"start_time": now.isoformat()}, "scheduled_start": lookback.isoformat()}],
        )

        active_patrol = dict(title="active", patrol_segments=[{"time_range": {"start_time": now.isoformat()}}])

        active_patrol2 = dict(
            title="active_control", patrol_segments=[{"time_range": {"start_time": active_control.isoformat()}}]
        )

        done_patrol = dict(
            title="done A", state="done", patrol_segments=[{"time_range": {"start_time": now.isoformat()}}]
        )
        done_patrol2 = dict(
            title="done B", state="done", patrol_segments=[{"time_range": {"start_time": active_control.isoformat()}}]
        )

        done_patrol3 = dict(
            state="done",
            patrol_segments=[
                {"time_range": {"start_time": active_control.isoformat()}, "patrol_type": "routine_patrol"}
            ],
        )
        ...
        expected = [
            ...
            "done B",
            "routine_patrol",
            "cancelled readytostart",
            "cancelled readytostart_control",
        ]
        results = [p.get("title") for p in response.data["results"]]
    ...
    def test_sort_overdue_readytostart_only_alphabetically(self):
        ...
        overdue_patrol3 = dict(title="A overdue", patrol_segments=[{"scheduled_start": third_lookback.isoformat()}])

        overdue_patrol4 = dict(
            patrol_segments=[
                {
                    "scheduled_start": third_lookback.isoformat(),
                    "patrol_type": "dog_patrol",
                    "leader": {
                        "content_type": "observations.subject",
                        "id": subj.id,
        ...
        ready_patrol = dict(title="C readytostart", patrol_segments=[{"scheduled_start": first_scheduled.isoformat()}])
        ready_patrol2 = dict(
            title="B readytostart", patrol_segments=[{"scheduled_start": second_scheduled.isoformat()}]
        )
        ready_patrol3 = dict(title="A readytostart", patrol_segments=[{"scheduled_start": third_scheduled.isoformat()}])

        ready_patrol4 = dict(
            patrol_segments=[{"scheduled_start": third_scheduled.isoformat(), "patrol_type": "dog_patrol"}]
        )
    ...
    def test_add_report_to_patrol_segment(self):
        ...
        location = PointField().to_internal_value({"longitude": -122.3607072, "latitude": 47.681731199999994})
        ...
        event_child = Event.objects.create(title="Event_A", event_type=et, location=location)
        EventRelationship.objects.add_relationship(event_collection, event_child, "contains")
    ...
    def test_add_patrol_segment_to_report(self):
        ...
        patrol_segment = dict(patrol_type="routine_patrol", patrol=patrol.id)
        ...
        self.force_authenticate(request, self.user)
        response = views.PatrolView.as_view()(request, id=patrol.id)
        self.assertEqual(1, len(response.data.get("patrol_segments")[0].get("events")))
    ...
    def test_patrolsegment_history_autoendtime(self):
        ...
        patrol_patrolsegment = dict(
            ...
            title="Patrol XYZ",
            patrol_segments=[
                {"patrol_type": "routine_patrol", "time_range": {"start_time": "2020-09-24T07:08:16.711000+03:00"}}
            ],
        )
        ...
        updated_patrol_patrolsg = dict(
            priority=200, patrol_segments=[{"id": patrol_sgs_id, "time_range": {"end_time": self.end_of_today}}]
        )
        ...
        response = views.PatrolView.as_view()(request, id=p.id)
        self.assertEqual(response.status_code, 200)
        self.assertTrue("Auto-End Time" in response.data["patrol_segments"][0]["updates"][0].get("message"))
            ...
            patrol_segments=[
                {
                    "id": patrol_sgs_id,
                    "scheduled_start": "2020-09-26T01:14:34.196502+03:00",
                    "time_range": {"end_time": "2020-09-29T07:08:16.711000+03:00"},
                }
            ],
        )
    ...
    def test_maintain_patrol_state(self):
        ...
        auto_done_message = [u for u in updates if "State is done" in u["message"]][0]
...
def test_patrols_materialized_view(django_assert_max_num_queries, client):
    ...
    sources = [
        {
            "model_name": "Model A",
            "manufacturer_id": "model-a-1",
        },
        {
            "model_name": "Model B",
            "manufacturer_id": "model-b-1",
        },
    ]
    ...
    for i, s in enumerate(sources):
        ...
        SubjectSource.objects.create(**{"subject": leader, "source": src, "assigned_range": range_overlaps[i]})
...
class TestPatrolFilter:
    ...
    def test_filter_by_tracked_subject_name(self, five_patrol_segment_subject, text, subject_group_with_perms):
    ...
    def test_filter_by_subject_list_with_one_value(
        ...
        client.app_user.permission_sets.add(view_patrol_permissionset)
        client.app_user.permission_sets.add(subject_group_with_perms.permission_sets.last())
        request = client.factory.get(client.api_base + f"/patrols/?filter={json.dumps(filters)}")
        client.force_authenticate(request, client.app_user)
        response = views.PatrolsView.as_view()(request)
        data = dict(response.data)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_filter_by_patrol_type_with_one_value(self, five_patrol_segment_patrol_type_uuid, patrol_type):
        filters = {"patrols_overlap_daterange": True, "patrol_type": [patrol_type]}
        client = HTTPClient()
        view_patrol_permissionset = PermissionSet.objects.get(name="View Patrols Permissions")
        client.app_user.permission_sets.add(view_patrol_permissionset)
        request = client.factory.get(client.api_base + f"/patrols/?filter={json.dumps(filters)}")
    ...
    @pytest.mark.parametrize(
        "patrol_type", [["00000000-0000-0000-0000-000000000001", "00000000-0000-0000-0000-000000000002"]]
    )
    ...
    def test_filter_by_patrol_status_list_with_one_value(self, five_patrol_segment, statuses):
        ...
        view_patrol_permissionset = PermissionSet.objects.get(name="View Patrols Permissions")
        client.app_user.permission_sets.add(view_patrol_permissionset)
        request = client.factory.get(client.api_base + f"/patrols/?status={'&status='.join(statuses)}")
        client.force_authenticate(request, client.app_user)
        response = views.PatrolsView.as_view()(request)
        data = dict(response.data)
...
class TestPatrolView:
    def test_create_patrol_with_past_end_date(self):
        ...
        patrol_data = {
            "patrol_segments": [
                {
                    "patrol_type": "routine_patrol",
                    "time_range": {"start_time": past_start_date.isoformat(), "end_time": past_end_date.isoformat()},
                }
            ],
    ...
    def test_update_patrol_with_past_end_date(self, five_patrol_segment):
        ...
        segment.time_range = DateTimeTZRange(lower=past_start_date, upper=past_end_date)
        segment.save()
        ...
        request = client.factory.patch(f"{client.api_base}/patrols/{patrol.id}", data=data)
        client.force_authenticate(request, client.app_user)
        response = views.PatrolView.as_view()(request, id=patrol.id)
        data = dict(response.data)
...
class TestPatrolModel:
    @pytest.mark.parametrize(
        "patrol_type", ["ff2f7da6-ade4-4dc1-bd7a-c2c5244017fa", "e62ca278-8687-455e-ba76-2c7dfa4d6b5f"]
    )
