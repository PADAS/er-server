...
from activity.exceptions import SCHEMA_ERROR_JSON_DECODE_ERROR, SchemaValidationError
...
class AutoResolveField(forms.fields.MultiValueField):
    ...
    error_message_hours = {
        "min_value": "Ensure 'value for hour' is greater than or equal to 1",
    ...
    def __init__(self, *args, **kwargs):
        _fields = [
            ...
            forms.fields.IntegerField(
                required=False, min_value=1, max_value=10000, error_messages=self.error_message_hours
            ),
        ]
...
def validate_schema_is_well_formed(schema):
    try:
        ...
        raise forms.ValidationError(f"Schema includes an invalid token {str(ne)}")
    except Exception:
...
class AlertRuleForm(forms.ModelForm):
    ...
    override_message = forms.CharField(
        widget=MonospaceTextWidget,
        required=False,
        help_text=_("If not empty, alerts will use this content instead of the default template."),
    )

    notification_methods = NotificationMethodSelectField(
        queryset=NotificationMethod.objects.all(),
        required=True,
        widget=FilteredSelectMultiple(verbose_name=_("Notification Methods"), is_stacked=False),
    )
...
class EventProviderForm(JSONFieldFormMixin, forms.ModelForm):
    ...
    external_event_url = forms.CharField(
        label="Provider Link",
        required=False,
        widget=TextInput(attrs={"size": "100"}),
...
class PatrolTypeForm(forms.ModelForm):
    icon = forms.CharField(required=False, label="Icon", widget=IconKeyInput(image_list_fn=get_icon_select_list))
...
class PatrolSegmentForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {"scheduled_start": "Scheduled start date", "scheduled_end": "Scheduled end date"}
    ...
    def __init__(self, *args, **kwargs):
        ...
        if instance:
            ...
            self.fields["tracked_subject"].initial = (
                instance.leader if instance.leader in dict(choices).values() else None
            )
