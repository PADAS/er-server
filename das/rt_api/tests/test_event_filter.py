...
class EventsFilterTestCase(BaseAPITest):
    def setUp(self):
        ...
        self.second_event_type = EventType.objects.create(display="Test Type 2", value="typetest2", category=category)
        self.event = Event.objects.create(title="test event", event_type=self.event_type, created_by_user=self.app_user)
    ...
    def test_event_filter_on_a_non_matching_event(self):
        ...
        new_event = Event.objects.create(
            title="new test event", event_type=self.second_event_type, created_by_user=self.app_user
        )
