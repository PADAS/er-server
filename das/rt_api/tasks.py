...
from activity.serializers import EventSerializer, PatrolSerializer
        ...
        for username, user_sids in user_sids_map.items():
            ...
            for sid in user_sids:
                ...
                if type_ == "delete_event":
                    ...
                    request = DummyRequest(user=user, http_method="GET", query_parameters=query_params)
                    ...
                    if event:
                        event_count = 1
                        try:
                            event_view.check_object_permissions(request=request, obj=event)
        ...
        for username, user_sids in user_sids_map.items():
            ...
            for sid in user_sids:
                if action == "delete_message":
                    emit_data = get_emit_data(
                        type=action, sid=sid, object_id=object_id, data={"type": action, "data": {"id": object_id}}
                    )
                else:
                    try:
                        ...
                        message_data = MessageSerializer(instance, context={"request": request}).data
                        emit_data = get_emit_data(
                            type=action, sid=sid, object_id=object_id, data={"type": action, "data": message_data}
                        )
        ...
        for username, user_sids in user_sids_map.items():
            ...
            for sid in user_sids:
                try:
                    ...
                    message_data = AnnouncementSerializer(instance, context={"request": request}).data
                    emit_data = get_emit_data(
                        type=action, sid=sid, object_id=object_id, data={"type": action, "data": message_data}
                    )
...
def check_redis_queues():
    ...
    for key, value in [
        ...
        update_gauge(metric="task_queue_length", value=value, tags=[f"queue:{key}"])
